...
const renderTestPane = async (redGPUContext) => {
	...
	const TEST_STATE = {
		RadialBlur: true,
		amount: view.postEffectManager.getEffectAt(0).amount,
		centerX: view.postEffectManager.getEffectAt(0).centerX,
		centerY: view.postEffectManager.getEffectAt(0).centerY,
		sampleCount: view.postEffectManager.getEffectAt(0).sampleCount,
	}
	...
	const amountControl = folder.addBinding(TEST_STATE, 'amount', {min: 0, max: 300}).on('change', (v) => {
		view.postEffectManager.getEffectAt(0).amount = v.value
	})
	...
	const sampleCountControl = folder.addBinding(TEST_STATE, 'sampleCount', {min: 4, max: 32}).on('change', (v) => {
		view.postEffectManager.getEffectAt(0).sampleCount = v.value
	})
};
