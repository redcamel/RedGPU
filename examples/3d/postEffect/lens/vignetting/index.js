...
const renderTestPane = async (redGPUContext) => {
	...
	const TEST_STATE = {
		Vignetting: true,
		smoothness: view.postEffectManager.getEffectAt(0).smoothness,
		size: view.postEffectManager.getEffectAt(0).size,
	}
	...
	const smoothnessControl = folder.addBinding(TEST_STATE, 'smoothness', {min: 0, max: 1}).on('change', (v) => {
		view.postEffectManager.getEffectAt(0).smoothness = v.value
	})
	const sizeControl = folder.addBinding(TEST_STATE, 'size', {min: 0, max: 1}).on('change', (v) => {
		view.postEffectManager.getEffectAt(0).size = v.value
	})
};
