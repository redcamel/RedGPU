...
RedGPU.init(
	...
	(redGPUContext) => {
		...
		renderTestPane(redGPUContext, mesh1, mesh2);
	},
	(failReason) => {
...
const renderTestPane = async (redGPUContext, mesh1, mesh2) => {
	...
	pane.addBinding(config, 'autoMove').on('change', (evt) => {
		config.autoMove = evt.value;
	});

	const mesh1Folder = pane.addFolder({title: 'Fixed Mesh Position', expanded: true});
	mesh1Folder.addBinding(config, 'mesh1X', {min: -8, max: 8, step: 0.1}).on('change', (evt) => {
		mesh1.setPosition(evt.value, config.mesh1Y, config.mesh1Z);
	});
	mesh1Folder.addBinding(config, 'mesh1Y', {min: -5, max: 5, step: 0.1}).on('change', (evt) => {
		mesh1.setPosition(config.mesh1X, evt.value, config.mesh1Z);
	});
	mesh1Folder.addBinding(config, 'mesh1Z', {min: -5, max: 5, step: 0.1}).on('change', (evt) => {
		mesh1.setPosition(config.mesh1X, config.mesh1Y, evt.value);
	});

	const mesh2Folder = pane.addFolder({title: 'Moving Mesh Position', expanded: true});
	mesh2Folder.addBinding(config, 'mesh2X', {min: -8, max: 8, step: 0.1}).on('change', (evt) => {
		if (!config.autoMove) {
			mesh2.setPosition(evt.value, config.mesh2Y, config.mesh2Z);
		}
	});
	mesh2Folder.addBinding(config, 'mesh2Y', {min: -5, max: 5, step: 0.1}).on('change', (evt) => {
		if (!config.autoMove) {
			mesh2.setPosition(config.mesh2X, evt.value, config.mesh2Z);
		}
	});
	mesh2Folder.addBinding(config, 'mesh2Z', {min: -5, max: 5, step: 0.1}).on('change', (evt) => {
		if (!config.autoMove) {
			mesh2.setPosition(config.mesh2X, config.mesh2Y, evt.value);
		}
	});
};