...
/**
 * [KO] Ring 메시 4종(Line / Triangle Grid / Triangle Radial / Point)과 타이틀 텍스트를 씬에 추가합니다.
 * [EN] Adds 4 Ring meshes (Line / Triangle Grid / Triangle Radial / Point) and a title text to the scene.
...
const createPrimitive = (redGPUContext, scene) => {
    ...
    // [KO] 공유 Ring 지오메트리 (innerRadius=0.5, outerRadius=1, thetaSegments=64, phiSegments=1)
    // [EN] Shared Ring geometry (innerRadius=0.5, outerRadius=1, thetaSegments=64, phiSegments=1)
...
const renderTestPane = (redGPUContext) => {
    ...
    // [KO] 설정값으로 Ring 지오메트리를 재생성하여 모든 메시에 적용
    // [EN] Rebuild Ring geometry from config and apply to all meshes
