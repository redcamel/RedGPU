...
const createPrimitive = (redGPUContext, scene) => {
    ...
    titleText.text = 'Customizable RoundedBox Primitive';
...
const renderTestPane = async (redGPUContext) => {
    ...
    ['width', 'height', 'depth'].forEach(prop => {
        geometryFolder.addBinding(config, prop, {min: 0.1, max: 5, step: 0.1}).on('change', updateGeometry);
    });
    geometryFolder.addBinding(config, 'radius', {min: 0, max: 2.5, step: 0.01}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'radiusSegments', {min: 1, max: 32, step: 1}).on('change', updateGeometry);
    ['widthSegments', 'heightSegments', 'depthSegments'].forEach(prop => {
        geometryFolder.addBinding(config, prop, {min: 1, max: 15, step: 1}).on('change', updateGeometry);
    });
