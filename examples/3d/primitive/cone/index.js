...
/**
 * [KO] Cone 메시 4종(Line / Triangle Grid / Triangle Diffuse / Point)과 타이틀 텍스트를 씬에 추가합니다.
 * [EN] Adds 4 Cone meshes (Line / Triangle Grid / Triangle Diffuse / Point) and a title text to the scene.
...
const createPrimitive = (redGPUContext, scene) => {
    ...
    // [KO] 공유 Cone 지오메트리 (radius=1, height=2, radialSegments=32, heightSegments=1)
    // [EN] Shared Cone geometry (radius=1, height=2, radialSegments=32, heightSegments=1)
...
const renderTestPane = (redGPUContext) => {
    ...
    // [KO] 설정값으로 Cone 지오메트리를 재생성하여 모든 메시에 적용
    // [EN] Rebuild Cone geometry from config and apply to all meshes
