...
/**
 * [KO] Plane 메시 4종(Line / Triangle Grid / Triangle Diffuse / Point)과 타이틀 텍스트를 씬에 추가합니다.
 * [EN] Adds 4 Plane meshes (Line / Triangle Grid / Triangle Diffuse / Point) and a title text to the scene.
...
const createPrimitive = (redGPUContext, scene) => {
    ...
    // [KO] 공유 Plane 지오메트리 (width=4, height=4, widthSegments=10, heightSegments=10)
    // [EN] Shared Plane geometry (width=4, height=4, widthSegments=10, heightSegments=10)
    ...
    title.text       = 'Customizable Plane Primitive';
    title.color      = '#ffffff';
    title.fontSize   = 96;
...
const renderTestPane = (redGPUContext) => {
    ...
    // [KO] 설정값으로 Plane 지오메트리를 재생성하여 모든 메시에 적용
    // [EN] Rebuild Plane geometry from config and apply to all meshes
