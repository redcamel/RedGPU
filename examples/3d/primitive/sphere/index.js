 ...
 *
 * [KO] Sphere 프리미티브 생성 및 모든 속성을 실시간으로 제어하는 방법을 보여줍니다.
 * [EN] Demonstrates how to create a Sphere primitive and control all its properties in real-time.
 */
...
const renderTestPane = async (redGPUContext) => {
    ...
    const geometryFolder = pane.addFolder({title: 'Sphere Properties', expanded: true});
    geometryFolder.addBinding(config, 'radius', {min: 0.5, max: 5, step: 0.1}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'widthSegments', {min: 3, max: 64, step: 1}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'heightSegments', {min: 3, max: 64, step: 1}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'phiStart', {min: 0, max: Math.PI * 2, step: 0.1}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'phiLength', {min: 0, max: Math.PI * 2, step: 0.1}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'thetaStart', {min: 0, max: Math.PI, step: 0.1}).on('change', updateGeometry);
    geometryFolder.addBinding(config, 'thetaLength', {min: 0, max: Math.PI, step: 0.1}).on('change', updateGeometry);
