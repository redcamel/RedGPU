...
/**
 * [KO] Sphere 메시 4종(Line / Triangle Grid / Triangle Diffuse / Point)과 타이틀 텍스트를 씬에 추가합니다.
 * [EN] Adds 4 Sphere meshes (Line / Triangle Grid / Triangle Diffuse / Point) and a title text to the scene.
...
const createPrimitive = (redGPUContext, scene) => {
    ...
    // [KO] 공유 Sphere 지오메트리 (radius=1, widthSegments=32, heightSegments=16)
    // [EN] Shared Sphere geometry (radius=1, widthSegments=32, heightSegments=16)
...
const renderTestPane = (redGPUContext) => {
    ...
    // [KO] 설정값으로 Sphere 지오메트리를 재생성하여 모든 메시에 적용
    // [EN] Rebuild Sphere geometry from config and apply to all meshes
