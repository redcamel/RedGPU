...
/**
 * [KO] Capsule 메시 4종(Line / Triangle Grid / Triangle Diffuse / Point)과 타이틀 텍스트를 씬에 추가합니다.
 * [EN] Adds 4 Capsule meshes (Line / Triangle Grid / Triangle Diffuse / Point) and a title text to the scene.
...
const createPrimitive = (redGPUContext, scene) => {
    ...
    // [KO] 공유 Capsule 지오메트리 (radius=0.5, height=1.0, radialSegments=32, heightSegments=1, capSegments=12)
    // [EN] Shared Capsule geometry (radius=0.5, height=1.0, radialSegments=32, heightSegments=1, capSegments=12)
    ...
    title.text       = 'Customizable Capsule Primitive';
    title.color      = '#ffffff';
    title.fontSize   = 96;
...
const renderTestPane = (redGPUContext) => {
    ...
    // [KO] 설정값으로 Capsule 지오메트리를 재생성하여 모든 메시에 적용
    // [EN] Rebuild Capsule geometry from config and apply to all meshes
    ...
    new RedGPUExampleHelper(redGPUContext, {
        guiCallback: (pane) => {
            ...
            geoFolder.addBinding(config, 'radius', {min: 0.1, max: 2, step: 0.1}).on('change', updateGeometry);
            ...
            geoFolder.addBinding(config, 'heightSegments', {min: 1, max: 32, step: 1}).on('change', updateGeometry);
            geoFolder.addBinding(config, 'capSegments', {min: 1, max: 32, step: 1}).on('change', updateGeometry);
