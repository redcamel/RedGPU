...
/**
 * [KO] TorusKnot 메시 4종(Line / Triangle Grid / Triangle Diffuse / Point)과 타이틀 텍스트를 씬에 추가합니다.
 * [EN] Adds 4 TorusKnot meshes (Line / Triangle Grid / Triangle Diffuse / Point) and a title text to the scene.
...
const createPrimitive = (redGPUContext, scene) => {
    ...
    // [KO] 공유 TorusKnot 지오메트리 (radius=1, tubeRadius=0.4, tubularSegments=64, radialSegments=8)
    // [EN] Shared TorusKnot geometry (radius=1, tubeRadius=0.4, tubularSegments=64, radialSegments=8)
...
const renderTestPane = (redGPUContext) => {
    ...
    // [KO] 설정값으로 TorusKnot 지오메트리를 재생성하여 모든 메시에 적용
    // [EN] Rebuild TorusKnot geometry from config and apply to all meshes
