...
const ExampleHelper = (_ => {
	...
	const setTestUI_Camera3D = (RedGPU, camera, open, gui) => {
		checkGUI();
		gui = gui || testHelperFolder;
		let rootFolder;
	...
	const setTestUI_PivotPoint2D = (RedGPU, redGPUContext, targetMesh, childMesh, open, gui) => {
		checkGUI();
		gui = gui || testHelperFolder;
		let rootFolder, folder;
		rootFolder = gui.addFolder('Pivot Test');
		...
		let pivotPoint1, pivotPoint2;
		pivotPoint1 = new RedGPU.Mesh(redGPUContext, new RedGPU.Plane(redGPUContext, 0.05,0.05), new RedGPU.ColorMaterial(redGPUContext))
		targetMesh.addChild(pivotPoint1);
		pivotPoint2 = new RedGPU.Mesh(redGPUContext, new RedGPU.Plane(redGPUContext, 0.05,0.05), new RedGPU.ColorMaterial(redGPUContext))
		childMesh.addChild(pivotPoint2);
		let HD_point1, HD_point2
		HD_point1 = _ => {
			pivotPoint1.x = targetMesh.pivotX/200
			pivotPoint1.y = targetMesh.pivotY/200
		}
		HD_point2 = _ => {
			pivotPoint2.x = childMesh.pivotX
			pivotPoint2.y = childMesh.pivotY
		...
		folder = rootFolder.addFolder('Parent Test');
		folder.open();
		folder.add(targetMesh, 'x', 0, 1920, 0.01).onChange(HD_point1);
		folder.add(targetMesh, 'y', 0, 1280, 0.01).onChange(HD_point1);
		folder.add(targetMesh, 'pivotX', -100, 100, 0.01).onChange(HD_point1);
		folder.add(targetMesh, 'pivotY', -100, 100, 0.01).onChange(HD_point1);
		folder = rootFolder.addFolder('Child Test');
		folder.open();
		folder.add(childMesh, 'x', -1, 1, 0.01).onChange(HD_point2);
		folder.add(childMesh, 'y', -1, 1, 0.01).onChange(HD_point2);
	...
	const setTestUI_GLTFLoader = (RedGPU, redGPUContext, tView, tScene, tCamera, open, gui) => {
		checkGUI();
		gui = gui || testHelperFolder;
		let rootFolder, folder;
		rootFolder = gui.addFolder('GLTFLoader');
		if (open) rootFolder.open();
		let tGLTF
		let gltfInfoList = [
		...
		let setGLTF = (v) => {
			console.log('Load Info', v)
			var t0 = v[0].split('/')
			var fileName = t0.pop()
			var path = t0.join('/') + '/'
			var distance = v[1]
			var speedDistance = v[2]
			distance = distance || 5
			speedDistance = speedDistance || 0.2
			...
			tScene.addChild(tGLTF['resultMesh'])
		}
		var testList = []
		gltfInfoList.forEach(function (v) {
			testList.push(v[0])
		})
		// var testList2 = []
		// gltfInfoList2.forEach(function (v) {
		// 	testList2.push(v[0])
		// })
		var testData = {
			sample1_basic: gltfInfoList[0][0],
			sample2_advanced: ''
		}

		rootFolder.add(
			testData, 'sample1_basic', testList,
		).onChange(function (v, index) {
			setGLTF(gltfInfoList[document.querySelector('select').selectedIndex])
		})
		// gui['gui'].add(
		// 	testData, 'sample2_advanced', testList2,
		// ).onChange(function (v, index) {
		// 	setGLTF(redGL, gltfInfoList2[document.querySelectorAll('select')[1].selectedIndex])
		// })
		setGLTF(gltfInfoList[0])
	}
	...
	const setTestUI_Camera2D = (RedGPU, redGPUContext, tMesh, open, gui) => {
		checkGUI();
		gui = gui || testHelperFolder;
		let rootFolder, folder;
		rootFolder = gui.addFolder('Mesh');
		...
		rootFolder.add(tMesh, 'cullMode', ["none", "back", "front",]);
		['x', 'y'].forEach(key => {
			rootFolder.add(tMesh, key, -100, 1000, 0.01);
			rootFolder.add(tMesh, 'scale' + key.toUpperCase(), -200, 200, 0.01)
		});
        rootFolder.add(tMesh, 'rotationZ', 0, 360, 0.01);
		const testData = {
			geometry: 'Sphere',
			material: 'ColorPhongMaterial'
		};
	...
	return {
		...
		setTestUI: (_ => {
			return (RedGPU, redGPUContext, view, scene, camera) => {
				...
				setTestUI_Camera3D(RedGPU, camera);
		...
		setTestUI_Camera3D: setTestUI_Camera3D,
		setTestUI_Camera2D: setTestUI_Camera2D,
		setTestUI_PivotPoint: setTestUI_PivotPoint,
		setTestUI_PivotPoint2D : setTestUI_PivotPoint2D,
		//
		...
		setTestUI_GLTFLoader: setTestUI_GLTFLoader
	};
})();
