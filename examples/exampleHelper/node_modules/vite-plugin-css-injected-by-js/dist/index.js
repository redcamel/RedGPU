...
export default function cssInjectedByJsPlugin({ cssAssetsFilterFunction, dev: { enableDev, removeStyleCode, removeStyleCodeFunction } = {}, injectCode, injectCodeFunction, injectionCodeFormat, jsAssetsFilterFunction, preRenderCSSCode, relativeCSSInjection, attributes, styleId, suppressUnusedCssWarning, topExecutionPriority, useStrictCSP, } = {}) {
    ...
    if (styleId) {
        warnLog('[vite-plugin-css-injected-by-js] The "styleId" option is deprecated and will be removed in 6.0.0, please use the "attributes" option instead with an "id" property.');
    }
    const plugins = [
        {
        ...
        },
        {
            ...
            async generateBundle(opts, bundle) {
                ...
                if (relativeCSSInjection) {
                    ...
                    await relativeCssInjection(bundle, assetsWithCss, buildCssCode, topExecutionPriorityFlag, config.build, isVirtualModuleUsed);
                    ...
                    const allCssAssets = Object.keys(bundle).filter((i) => bundle[i].type == 'asset' && bundle[i].fileName.endsWith('.css'));
                    unusedCssAssets = allCssAssets.filter((cssAsset) => !cssAssets.includes(cssAsset));
                    await globalCssInjection(bundle, cssAssets, buildCssCode, jsAssetsFilterFunction, topExecutionPriorityFlag, config.build, isVirtualModuleUsed);
                }
