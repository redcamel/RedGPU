...
/**
 * [KO] Gaussian Blur 예제
 * [EN] Gaussian Blur example
 *
 * [KO] 가우시안 블러(Gaussian Blur) 포스트 이펙트를 시연합니다.
 * [EN] Demonstrates Gaussian blur post effect.
 */
...
RedGPU.init(
    ...
    (redGPUContext) => {
        ...
        viewEffect.postEffectManager.addEffect(new RedGPU.PostEffect.GaussianBlur(redGPUContext));
...
const renderTestPane = async (redGPUContext, targetView) => {
    ...
    createPostEffectLabel('GaussianBlur', redGPUContext.detector.isMobile)
    ...
    // GaussianBlur 토글
    folder.addBinding(TEST_STATE, 'GaussianBlur').on('change', (v) => {
        if (v.value) {
    ...
    const sizeControl = folder.addBinding(TEST_STATE, 'size', {min: 0, max: 50}).on('change', (v) => {
        targetView.postEffectManager.getEffectAt(0).size = v.value
    })
};