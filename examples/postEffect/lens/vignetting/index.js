...
RedGPU.init(
    ...
    (redGPUContext) => {
        ...
        viewEffect.postEffectManager.addEffect(new RedGPU.PostEffect.Vignetting(redGPUContext));
...
const renderTestPane = async (redGPUContext, targetView) => {
    ...
    const TEST_STATE = {
        Vignetting: true,
        smoothness: targetView.postEffectManager.getEffectAt(0).smoothness,
    ...
    folder.addBinding(TEST_STATE, 'Vignetting').on('change', (v) => {
        if (v.value) {
            ...
            effect.smoothness = TEST_STATE.smoothness;
    ...
    const sizeControl = folder.addBinding(TEST_STATE, 'size', {min: 0, max: 1}).on('change', (v) => {
