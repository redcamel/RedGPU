...
*   **비고**: 성능 분석용 필드(`viewRenderTime`, `numDrawCalls` 등)는 렌더링 로직과는 무관하나 엔진 최적화 및 프로파일링을 위해 의도적으로 유지함.
...
| :-- | :--- | :---: | :---: | :---: | :--- |
| **P1** | 클러스터 조명 로직 이관 | 높음 | 낮음 | 진행 중 | `View3D.ts`, `SystemUniformUpdater.ts` |
| **P1** | 셰이더 상수 통합 | 높음 | 보통 | 대기 | `src/**/*.wgsl` |
| **P1** | **시간 데이터 구조체화** | 높음 | 낮음 | ✅ 완료 | `Time.wgsl`, `SystemUniformUpdater.ts` |
| **P1** | **투영 행렬 구조체화** | 높음 | 낮음 | ✅ 완료 | `Projection.wgsl`, `SystemUniformUpdater.ts` |
| **P2** | 정점 셰이더 I/O 명칭 정규화 | 보통 | 낮음 | 대기 | `src/display/**/shader/*.wgsl` |
