...
<table style="width: 100%;">
  ...
  <tbody>
    ...
    <tr>
      <td><b>Phong Light</b></td>
      <td><code>lighting.getPhongLight</code></td>
      <td align="center">Frag Only</td>
      <td align="center">✅ 완료</td>
      <td><b>[퐁 조명 통합]</b> 모든 광원 유형(평행광, 점광원 등)에 공통 적용 가능한 퐁 라이팅 계산 엔진.</td>
    </tr>
  </tbody>
</table>
...
- **[퐁 조명 통합]**: `lighting.getPhongLight`를 통해 `PhongMaterial` 내의 모든 조명 계산 로직을 단일화 및 모듈화 완료.
- **[재질 통합]**: `pbrMaterial`, `phongMaterial` 내 조명 루프 구조 일치화 및 `NdotL` 네이밍 컨벤션 정규화.
- **[대기 태양광 연동]**: `skyAtmosphere.getAtmosphereSunLight`를 통해 대기 시스템의 태양 정보를 모든 머티리얼 조명 연산에 통합 완료.
- **[물리적 태양색 구현]**: 투과율 LUT(`transmittanceTexture`)를 전역 바인딩(Binding 15)에 추가하고, 태양 고도와 카메라 고도에 따라 실시간으로 변하는 물리적 태양색 계산 로직 적용 완료.
- **[IBL 대기 동기화]**: `skyViewTexture` 전역 바인딩(Binding 16) 추가 및 PBR 재질의 IBL 샘플링 결과에 실시간 대기 투과율/산란광 필터를 적용하여 환경 정합성 완성.
...
<table style="width: 100%;">
  ...
  <tbody>
    <tr>
      <td><b>Aerial Perspective</b></td>
      <td><code>skyAtmosphere.getAerialPerspective</code></td>
      <td align="center">Frag Only</td>
      <td align="center">✅ 완료</td>
      <td><b>[공중 투시]</b> 대기 산란 LUT를 이용한 거리별 안개 및 산란광 합성. (성능을 위해 호출부에서 활성 여부 체크 권장)</td>
    </tr>
    <tr>
      <td><b>Atmosphere Sun</b></td>
      <td><code>skyAtmosphere.getAtmosphereSunLight</code></td>
      <td align="center">Frag Only</td>
      <td align="center">✅ 완료</td>
      <td><b>[대기 태양광]</b> SkyAtmosphere 시스템의 태양 정보(방향, 강도 등) 추출 구조체 및 함수.</td>
    </tr>
    <tr>
      <td><b>Height Fog</b></td>
      <td><code>math.getHeightFogFactor</code></td>
      <td align="center">Common</td>
      <td align="center">High</td>
      <td><b>[환경 감쇄]</b> 고도 기반 안개 수식의 수치 안정화 및 모듈화 예정.</td>
    </tr>
  </tbody>
</table>

#### 📂 상세 적용 이력 (Atmosphere)
- **[네임스페이스 분리]**: `skyAtmosphere` 전용 네임스페이스 및 폴더 구조 신설로 대기 관련 시스템 함수 관리 일원화.
- **[공중 투시 통합]**: `skyAtmosphere.getAerialPerspective` 구현 및 `colorMaterial`, `phongMaterial`, `pbrMaterial` 등 모든 재질에 적용 완료.
- **[물리적 조명 연동]**: 대기 태양의 방향과 강도를 시스템 유니폼에 공유하여 실제 물체의 조명(Phong/PBR)에 반영하도록 아키텍처 확장 완료.
...
<table style="width: 100%;">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>systemStruct.OutputFragment</code></td>
      <td align="center">Frag Only</td>
      <td align="center">✅ 완료</td>
    ...
    <tr>
      <td><b>Instancing Struct</b></td>
      <td><code>InstancingMesh.InstanceUniforms</code></td>
      <td align="center">Vert/Comp</td>
      <td align="center">✅ 완료</td>
      <td><b>[인스턴싱 표준]</b> 16바이트 정렬 최적화 및 그룹 노말 행렬이 포함된 정규화된 인스턴싱 데이터 구조.</td>
    </tr>
    <tr>
      <td><b>Instancing Entry</b></td>
      <td><code>entryPoint.instancing.vertex / culling</code></td>
      <td align="center">Vert/Comp</td>
      <td align="center">✅ 완료</td>
      <td><b>[인스턴싱 엔트리]</b> 그룹 변환 행렬이 교정된 물리 기반 인스턴싱 렌더링 및 GPU 컬링 파이프라인.</td>
    </tr>
  </tbody>
</table>
...
- **[인스턴싱 시스템 정규화]**: `InstancingMesh`의 데이터 레이아웃을 16바이트 단위로 재정렬하고, 그룹 노말 행렬(`instanceGroupNormalModelMatrix`)을 도입하여 회전 및 이동 시의 물리적 정합성을 확보함. 버텍스 셰이더 내 월드 좌표 계산 시 그룹 행렬 누락 문제를 해결하여 공중 투시 효과를 정상화함.
...
- **[구조체 표준화]**: `OutputFragment`, `OutputShadowData`, `SYSTEM_UNIFORM`, `POST_EFFECT_SYSTEM_UNIFORM`을 `src/systemCodeManager/shader/systemStruct/`로 집결하여 시스템 차원의 데이터 규격을 통합 관리.
- **[파일명 정규화]**: 시스템 전역 유니폼 파일명을 대문자로 통일(`SYSTEM_UNIFORM.wgsl`, `POST_EFFECT_SYSTEM_UNIFORM.wgsl`)하여 중요도와 가시성 확보.
...
- **[레거시 별칭 제거]**: `SystemCodeManager.ts` 내의 중복된 레거시 직접 참조(`calcTintBlendMode`, `getBillboardMatrix` 등)를 전량 제거하고 네임스페이스 기반 경로로 셰이더 코드 전수 교체 완료.
...
---
**최종 업데이트:** 2026-02-24
**상태:** 조명, 그림자, KHR 확장 인프라 표준화 및 공중 투시 통합 완료
**프로젝트:** RedGPU
