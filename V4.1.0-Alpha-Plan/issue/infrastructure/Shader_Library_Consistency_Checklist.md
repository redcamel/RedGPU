...
- **2026-02-23**: 디스플레이스먼트 라이브러리 리팩토링 및 좌표계 보정 로직 반영. 모든 레거시 인덱스 파일(`SystemCode`, `SystemVertexCode`) 삭제 및 `SystemCodeManager` 기반 전면 일원화. 엔진 전반의 TypeScript 직접 임포트 정규화 및 시스템 유니폼 파일 구조 정규화 작업 완료. 조명 구조체(`DirectionalLight`, `AmbientLight`) 및 투영 행렬 구조체(`Projection`) 파일 분리 완료. `SYSTEM_UNIFORM` 및 `POST_EFFECT_SYSTEM_UNIFORM` 내 구조체 통합과 `SystemUniformUpdater`를 통한 업데이트 로직 모듈화 완료. 특히 `View3D`와 `PostEffectManager` 간의 투영 업데이트 로직을 100% 동기화하여 TAA 및 SSR 연산의 정합성을 확보함. 쉐이더 코드 내 투영 행렬 접근 경로(`projection.XXXX`) 일괄 갱신 완료.
