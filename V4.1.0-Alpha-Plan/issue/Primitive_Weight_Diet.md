...
| 항목 | 표준 정의 | 시각적 설명 |
| :--- | :--- | :--- |
| **0도 시작점** | **3시 방향 (+X축)** | 모든 회전체의 thetaStart: 0은 오른쪽(+X)에서 시작합니다. |
| **회전 방향** | **상단 조감 기준 반시계 회전 (CCW)** | 정점이 생성되는 궤적은 3시(+X) → 12시(-Z) → 9시(-X) → 6시(+Z) 순서입니다. |
| **정면 위치** | **u = 0.75 지점** | 텍스처 가로축(U)의 75% 지점이 카메라를 바라보는 정면(+Z축)입니다. |
...
| **수평 흐름** | **우측 → 정면 → 좌측** | 반시계 방향 회전에 따라 텍스처가 오른쪽에서 정면을 거쳐 왼쪽으로 감쌉니다. |
