...
- [ ] **`PrimitiveUtils` 신설 및 로직 이관**:
  - [x] `generatePlaneData`: `Box`, `Plane`, `Ground` 클래스의 평면 기반 데이터 생성 로직을 통합 완료. (오른손 좌표계 및 V-Down UV 표준 준수)
  - [x] `generateCircleData`: `Circle`, `Cylinder`(상/하단 캡), `Torus`(단면) 클래스에 벡터 기반 범용 원형 데이터 생성 엔진 적용 완료. **(기존 Circle UV 뒤집힘 버그 수정됨)**
  - [ ] `generateGridIndices`: `Box`, `Plane`, `Ground`, `Sphere`, `Cylinder`, `Capsule`, `Torus`, `TorusKnot` 등 모든 격자형 도형의 인덱스 생성 로직 공통화.
  - [ ] `interleavePacker`: `Box`, `Sphere`, `Plane`, `Ground`, `Cylinder`, `Capsule`, `Circle`, `Torus`, `TorusKnot` 등 모든 프리미티브의 데이터 패킹 시 오차 방지를 위한 구조화된 헬퍼 도입.
...
### 3. 공통 기반 기술
- [ ] **Interleave Format 표준화**:
  - `Position(3), Normal(3), UV(2), Tangent(4)` 레이아웃을 전역 표준으로 확정.
- [ ] **Shader Chunks 활용**:
...
- **`Circle` UV 뒤집힘 수정**: 기존에 상하가 반전되어 생성되던 Circle의 UV 좌표를 V-Down(Top-Left) 표준에 맞게 교정.
- **`Cylinder` 캡 UV 통합**: 상/하단 캡(Cap)의 UV를 `generateCircleData`를 통해 중앙 집중식 매핑(0.5, 0.5 중심)으로 표준화.
...
- **상용 엔진 표준 정렬 (Industry Standard Alignment)**:
  - `Cylinder` 이음새(Seam) 위치 교정: 텍스처 이음새를 뒤쪽(-Z)으로 배치하여 정면에서 중앙이 보이도록 언리얼 엔진 스타일 표준 적용.
  - 몸체와 캡의 UV 동기화: 벡터 기반 생성을 통해 몸체와 캡의 텍스처 방향을 논리적으로 일치시킴.
...
| **Primitive Utils** | 0 | +300 | **N/A** | 공통 로직 집중화 (신규 생성) |
