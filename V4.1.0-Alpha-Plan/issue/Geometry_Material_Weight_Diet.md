...
- [x] **`PrimitiveUtils` 신설 및 로직 이관**:
  ...
  - [x] `calculateTangents`: 공통 수학 유틸리티 기반의 탄젠트 계산 로직을 통합하여 모든 프리미티브에 적용 완료.
...
- [ ] **탄젠트 데이터 활용 및 노멀 맵 셰이더 최적화**:
  - 기존 셰이더 내 탄젠트 실시간 계산 로직을 제거하고, 정점 데이터에 포함된 `Tangent` 속성을 사용하도록 WGSL 코드 업데이트.
  - TBN 행렬 구성 시 정점 탄젠트 정보를 활용하여 연산 부하 감소 및 정확도 향상.
- [ ] **`ABaseMaterial` 기능 확장**:
  - 텍스처 및 샘플러 바인딩 관리를 베이스 클래스에서 일관되게 처리하여 하위 클래스의 구현 복잡도 완화.

### 3. 공통 기반 기술
- [x] **Interleave Format 표준화 (12-Float)**:
...
## 🔍 UV 정합성 및 탄젠트 최적화 내역 (Consistency Improvements)
...
- **탄젠트 데이터 포함 (Tangent Inclusion)**: 
  - 렌더링 정확도(UV 미러링, 이음새 부드러움) 및 프래그먼트 셰이더 연산 부하 감소(ALU 최적화)를 위해 12-플로트 구조 유지 확정.
  - 모든 프리미티브 생성 시 `src/math/calculateTangents.ts`를 통한 정밀 탄젠트 데이터 생성 로직 적용.
...
| **Primitive Utils** | 0 | +450 | **N/A** | 공통 로직 집중화 (신규 생성) |
