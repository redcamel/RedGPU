...
디버깅 편의성을 위해 모든 GPU 리소스(Buffer, Texture, Pipeline, BindGroup 등)의 라벨명을 `SkyAtmosphere_` 접두어로 시작하도록 통일했습니다.

| 진행 상황 | 대상 컴포넌트 | 주요 변경 사항 | 기대 효과 |
| :---: | :--- | :--- | :--- |
| **완료** | 전체 모듈 | `SKY_ATMOSPHERE_*` ➡️ `SkyAtmosphere_*` (규칙적인 PascalCase/Snake_Case 적용) | WebGPU Inspector 등 디버깅 툴에서 리소스 식별 용이성 증대 |
| **완료** | 전체 모듈 | 라벨이 누락되었거나 불명확한 `createBindGroup`, `createBuffer` 등에 명확한 라벨 추가 | 런타임 에러 추적 시 리소스 위치 및 용도 즉시 파악 가능 |
