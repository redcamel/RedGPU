...
*   **해결**: 중복 곱셈을 제거하고 표준 물리 합성 공식($Final = Scene \times Transmittance + InScattering$)을 적용했습니다.

### 2.3 매핑 안정화 및 정밀도 향상
*   **해결**: `getSkyViewUV` 호출 시 지평선 계산을 위해 높이를 0 이상으로 클램핑하는 `mappingH` 로직을 도입하여 지하 시점에서도 LUT 샘플링이 깨지지 않도록 보정했습니다.
