...
(async function () {
	...
	let redGPU = new RedGPU(cvs, glslang,
		function () {
			...
			// tView.postEffect.addEffect(new RedPostEffect_Invert(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_Gray(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_BrightnessContrast(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_Threshold(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_HueSaturation(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_Pixelize(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_HalfTone(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_Blur(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_BlurX(redGPU))
			// tView.postEffect.addEffect(new RedPostEffect_BlurY(redGPU))
			...
			// testMat = new RedEnvironmentMaterial(redGPU, null, testCubeTexture, null, null, null, null)
			testMat = new RedEnvironmentMaterial(redGPU, testTextureList[0], testCubeTexture, null, null, null, null)
			testMat = new RedEnvironmentMaterial(redGPU, testTextureList[0], testCubeTexture, testTextureList[1], testTextureList[2], testTextureList[3], testTextureList[4])
			testMat.environmentPower = 1
			testMat.shininess = 6
			...
			while(i--){
				...
				testMesh.scaleX = testMesh.scaleY = testMesh.scaleZ = Math.random()*10
				tScene.addChild(testMesh)
			}

			let renderer = new RedRender();
			let render = function (time) {
				testMat.environmentPower = Math.abs(Math.sin(time / 500))
