...
따라서 인스펙터는 반드시 **동적 임포트(Dynamic Import)** 방식으로 불러와야 합니다.
...
프로젝트에서 RedGPU 파일을 로컬에 다운로드하여 사용하는 경우, 다음과 같이 현재 파일의 URL을 기준으로 상대 경로를 지정합니다.
...
> [!NOTE]
> 인스펙터는 **개발 환경 전용**입니다. 프로덕션 배포 시에는 `inspector.useDebugPanel = true;` 코드를 제거하거나, 인스펙터를 불러오는 동적 임포트 블록 자체를 제거하는 것을
> 권장합니다.

## 다음 학습 추천

- **[RedGPU Context](../context/index.md)** : 엔진 컨텍스트의 고급 설정 및 리소스 관리.
- **[후처리 이펙트](../post-effect/index.md)** : 씬에 시각적 효과를 더하는 포스트 프로세싱.
- **[LOD](../lod/index.md)** : 거리에 따른 메시 품질 자동 조절로 성능 최적화.
