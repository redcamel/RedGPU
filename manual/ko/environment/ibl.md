...
**난반사 텍스처**(Diffuse Texture) 는 환경 맵의 전체적인 색상 정보를 아주 부드럽게 블러(Blur) 처리한 데이터입니다.
- **역할**: 물체의 기본 명암과 주변광(Ambient) 역할을 하며, 직접적인 광원이 없는 곳에서도 자연스러운 색감을 부여합니다.
- **비유**: 안개가 낀 날 전체적으로 퍼지는 은은한 빛과 같습니다.

### 1.2 정반사 텍스처

**정반사 텍스처**(Specular Texture) 는 환경 맵의 고해상도 정보를 유지하면서 거칠기(Roughness) 단계별로 미리 계산된 데이터입니다.
...
IBL은 특히 **물리 기반 렌더링**(PBR) 재질을 사용하는 GLTF 모델에서 그 진가를 발휘합니다. 금속 재질의 모델은 주변 환경을 반사하지 않으면 단순한 회색으로 보일 수 있지만, IBL을 적용하면 비로소 금속 특유의 광택을 가지게 됩니다.
...
- **난반사 텍스처** 는 전역적인 조명과 색감을 제공합니다.
- **정반사 텍스처** 는 금속성 재질의 사실적인 반사를 담당합니다.
