...
Defined in: [src/systemCodeManager/ShaderLibrary.ts:604](https://github.com/redcamel/RedGPU/blob/3f2479cb923d5cf594209ed38cdda688909e57d0/src/systemCodeManager/ShaderLibrary.ts#L604)

빌보드 계산 결과 구조체입니다.

```wgsl
struct BillboardResult {
    position: vec4<f32>,       // 클립 공간 좌표
    vertexPosition: vec3<f32>, // 뷰 공간 좌표
    vertexNormal: vec3<f32>,   // 뷰 공간 법선
}

/**
 * 빌보드 및 픽셀 크기 모드를 지원하는 공통 정점 변환 결과 데이터를 리턴합니다.
 * @param input_position 입력 정점 위치
 * @param input_normal 입력 정점 법선
 * @param modelMatrix 모델 행렬
 * @param viewMatrix 뷰 행렬
 * @param projectionMatrix 투영 행렬
 * @param resolution 화면 해상도
 * @param useBillboard 빌보드 사용 여부 (1u: 사용)
 * @param usePixelSize 픽셀 크기 모드 사용 여부 (1u: 사용)
 * @param pixelSize 픽셀 크기
 * @param renderRatioX 렌더링 가로 비율
 * @param renderRatioY 렌더링 세로 비율
 * @returns 빌보드 계산 결과 데이터
 *
 */
