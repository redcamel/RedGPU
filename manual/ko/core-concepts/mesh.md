...
**Mesh**는 RedGPU의 3D 공간에서 시각적으로 표현되는 가장 기본적인 객체 단위입니다. 뼈대에 해당하는 **지오메트리**와 피부에 해당하는 **머티리얼**이 결합되어 하나의 완성된 가시 객체를 형성합니다.

## 1. 메시의 구성 요소

메시는 독립적으로 존재할 수 없으며, 반드시 다음 두 요소가 결합되어야 합니다.

- **Geometry** (지오메트리): 점, 선, 면으로 이루어진 물체의 물리적 **형태**입니다.
- **Material** (머티리얼): 색상, 질감, 반사광 등 물체의 **표면** 특성을 결정합니다.
...
RedGPU는 별도의 외부 모델 없이 즉시 사용할 수 있는 다양한 **RedGPU.Primitive**를 제공합니다.
...
```javascript
// 빨간색 불투명 재질 생성
const redMat = new RedGPU.Material.ColorMaterial(redContext, '#ff0000');

// 파란색 반투명 재질 생성 (alpha: 0.5)
const blueAlphaMat = new RedGPU.Material.ColorMaterial(redContext, '#0000ff');
blueAlphaMat.alpha = 0.5; 
```

## 4. 메시 제어 (Transformation)

생성된 **Mesh** 객체는 3D 공간 내에서 자유롭게 변환할 수 있는 속성을 가집니다.

- **Position** (`x`, `y`, `z`): 공간 내의 위치를 지정합니다.
...
RedGPU.init(canvas, (redContext) => {
    ...
    const floor = new RedGPU.Display.Mesh(
        redContext,
        new RedGPU.Primitive.Ground(redContext, 50, 50),
    ...
    const torus = new RedGPU.Display.Mesh(
        redContext,
        new RedGPU.Primitive.Torus(redContext, 3, 0.5),
        new RedGPU.Material.ColorMaterial(redContext, '#ffcc00')
    );
    torus.y = 2;
    ...
    const box = new RedGPU.Display.Mesh(
        redContext,
        new RedGPU.Primitive.Box(redContext),
    ...
    const knot = new RedGPU.Display.Mesh(
        redContext,
        new RedGPU.Primitive.TorusKnot(redContext, 1.5, 0.4),
        new RedGPU.Material.ColorMaterial(redContext, '#ff4d4f')
    );
    knot.x = 6; knot.y = 2.5;
    scene.addChild(knot);
...
- 자체적인 **scale**, **rotation**, **position** 속성으로 3D 공간 내 변환을 직접 제어합니다.
- **Scene**에 추가됨으로써 비로소 공간의 구성 요소로 인정되어 화면에 렌더링됩니다.
