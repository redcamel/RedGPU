...
3D 공간을 화면에 출력하기 위해서는 세상을 바라보는 눈인 **Camera**가 필요합니다. 카메라는 3D 좌표를 2D 모니터 화면으로 변환해주는 역할을 합니다.

## 1. PerspectiveCamera (원근 카메라)

RedGPU의 **RedGPU.Camera.PerspectiveCamera**는 실제 인간의 눈이나 카메라 렌즈와 동일한 방식으로 작동합니다. 멀리 있는 물체는 작게, 가까운 물체는 크게 렌더링하여 자연스러운 입체감을 제공합니다.
...
카메라를 특정 위치에 두었더라도, 정작 엉뚱한 곳을 보고 있다면 화면에 아무것도 나오지 않을 수 있습니다. `lookAt()` 메서드를 사용하면 카메라가 특정 좌표를 정면으로 응시하도록 설정할 수 있습니다.
...
카메라의 위치를 옮기면서 원점을 바라보는 예제입니다.

```javascript
import * as RedGPU from "https://redcamel.github.io/RedGPU/dist/index.js";

RedGPU.init(document.getElementById('redgpu-canvas'), (redContext) => {
    ...
    scene.addChild(new RedGPU.Display.Mesh(
        redContext, new RedGPU.Primitive.Ground(redContext, 10, 10),
        new RedGPU.Material.ColorMaterial(redContext, '#ffffff')
    ));
    const box = new RedGPU.Display.Mesh(
        redContext, new RedGPU.Primitive.Box(redContext),
...
아래 예제는 사용자의 조작 없이 코드로 카메라의 위치와 시점을 실시간으로 변경하는 모습을 보여줍니다.
...
- **lookAt()** 메서드는 카메라가 바라볼 대상(좌표)을 지정하는 가장 직관적인 방법입니다.
