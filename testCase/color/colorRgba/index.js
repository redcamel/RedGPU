...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Function Tests - create & getter',
	(runner) => {

		runner.defineTest('Test RedGPU.Color.ColorRGBA creation from [255, 255, 255]', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FFFFFF', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
			const expectedRGB = [255, 255, 255];
			run(colorRGB.r === expectedRGB[0] && colorRGB.g === expectedRGB[1] && colorRGB.b === expectedRGB[2]);
		}, true);

		runner.defineTest('Test RedGPU.Color.ColorRGBA creation from [255, 255, 0]', function (run) {
		...
		runner.defineTest('Test RedGPU.Color.ColorRGBA rgb getter for [255, 255, 255]', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FFFFFF', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
			const expectedRGB = [255, 255, 255];
		...
		runner.defineTest('Test RedGPU.Color.ColorRGBA rgb getter for [255, 255, 0]', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FF0', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
			const expectedRGB = [255, 255, 0];
			run(colorRGB.rgb[0] === expectedRGB[0] && colorRGB.rgb[1] === expectedRGB[1] && colorRGB.rgb[2] === expectedRGB[2]);
		}, true);
		runner.defineTest('Test RedGPU.Color.ColorRGBA hex getter for #FFFFFF', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FFFFFF', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
		...
		runner.defineTest('Test RedGPU.Color.ColorRGBA hex getter for #FFFF00', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FFFF00', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
			const expectedHex = "#FFFF00";
			run(colorRGB.hex === expectedHex);
		}, true);
		runner.defineTest('Test RedGPU.Color.ColorRGBA rgbNormal getter for [255, 255, 255]', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FFFFFF', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
		...
		runner.defineTest('Test RedGPU.Color.ColorRGBA rgbNormal getter for [255, 255, 0]', function (run) {
			const rgbArray = RedGPU.Color.convertHexToRgb('#FFFF00', true);
			const colorRGB = new RedGPU.Color.ColorRGBA(rgbArray[0], rgbArray[1], rgbArray[2]);
		...
		runner.defineTest('Test default value when RedGPU.Color.ColorRGBA creation with undefined values', function (run) {
			let passYn, error;
			try {
				const colorRGB = new RedGPU.Color.ColorRGBA(undefined, undefined, undefined);
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA setColorByRGB Function Tests',
	(runner) => {
		runner.defineTest('Test setColorByRGB for [255, 255, 255]', function (run) {
			const colorRGB = new RedGPU.Color.ColorRGBA(0, 0, 0);
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA setColorByHEX Function Tests',
	(runner) => {
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA setColorByRGBString Function Tests',
	(runner) => {
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Creation Error Cases',
	(runner) => {

		runner.defineTest('Test error when RedGPU.Color.ColorRGBA creation with negative values', function (run) {
			let passYn, error;
			try {
				const colorRGB = new RedGPU.Color.ColorRGBA(-10, -20, -30);
				passYn = true;
			} catch (e) {
				passYn = false;
				error = e;
		...
		runner.defineTest('Test error when RedGPU.Color.ColorRGBA creation with non-integer values', function (run) {
			let passYn, error;
			try {
				const colorRGB = new RedGPU.Color.ColorRGBA(0.1, 0.2, 0.3);
				passYn = true;
			} catch (e) {
				passYn = false;
				error = e;
		...
		runner.defineTest('Test error when RedGPU.Color.ColorRGBA creation with null values', function (run) {
			let passYn, error;
			try {
				const colorRGB = new RedGPU.Color.ColorRGBA(null, null, null);
				passYn = true;
			} catch (e) {
				passYn = false;
				error = e;
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Error Cases',
	(runner) => {
		runner.defineTest('Test error when RedGPU.Color.ColorRGBA creation with invalid hex color code', function (run) {
			try {
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Function Tests - create & getter',
	(runner) => {
		runner.defineTest('Test RedGPU.Color.ColorRGBA creation from [255, 255, 255, 1]', function (run) {
			const colorRGBA = new RedGPU.Color.ColorRGBA(255, 255, 255, 1);
		...
		runner.defineTest('Test RedGPU.Color.ColorRGBA alpha property for [255, 255, 255, 1]', function (run) {
			const colorRGBA = new RedGPU.Color.ColorRGBA(255, 255, 255, 1);
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Failure Test Group - Invalid alpha values',
	(runner) => {
		runner.defineTest('Test error when creating RedGPU.Color.ColorRGBA with invalid alpha value - less than 0', function (run) {
			try {
				const colorRGBA = new RedGPU.Color.ColorRGBA(255, 255, 255, -0.1); // Alpha value is less than 0.0
		...
		runner.defineTest('Test error when creating RedGPU.Color.ColorRGBA with invalid alpha value - greater than 1', function (run) {
			try {
				const colorRGBA = new RedGPU.Color.ColorRGBA(255, 255, 255, 1.1); // Alpha value is greater than 1.0
				run(true); // If it gets to here without throwing an error, the test fails.
			} catch (err) {
				run(false, err); // If an error is thrown, the test passes.
			}
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Function Tests - getters',
	(runner) => {
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Function Tests - setColorByRGBAString',
	(runner) => {
		runner.defineTest('Test setColorByRGBAString with valid RGBA string', function (run) {
			const colorRGBA = new RedGPU.Color.ColorRGBA(0, 0, 0, 0);
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Function Tests - setColorByRGBA (Success Group)',
	(runner) => {
...
redUnit.testGroup(
	'RedGPU.Color.ColorRGBA Function Tests - setColorByRGBA (Failure Group)',
	(runner) => {
