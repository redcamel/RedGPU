...
redUnit.testGroup(
    ...
    (runner) => {
        runner.defineTest('Success: Valid #ff0000', (run) => {
            try { run(RedGPU.RuntimeChecker.isHexColor('#ff0000')); } catch (e) { run(false, e); }
        }, true);
        runner.defineTest('Success: Valid 0xff0000', (run) => {
            try { run(RedGPU.RuntimeChecker.isHexColor('0xff0000')); } catch (e) { run(false, e); }
        }, true);
        runner.defineTest('Failure: ff0000 (no prefix)', (run) => {
            try { run(RedGPU.RuntimeChecker.isHexColor('ff0000')); } catch (e) { run(false, e); }
        }, false);
        runner.defineTest('Failure: invalid Type - number', (run) => {
            try { RedGPU.RuntimeChecker.isHexColor(123); run(true); } catch (e) { run(false, e); }
        }, false);
    }
);
...
redUnit.testGroup(
    ...
    (runner) => {
        runner.defineTest('Success: Positive integer (10)', (run) => {
            try { run(RedGPU.RuntimeChecker.isUint(10)); } catch (e) { run(false, e); }
        }, true);
        runner.defineTest('Failure: Float (10.5)', (run) => {
            try { run(RedGPU.RuntimeChecker.isUint(10.5)); } catch (e) { run(false, e); }
        }, false);
        runner.defineTest('Failure: NaN', (run) => {
            try { run(RedGPU.RuntimeChecker.isUint(NaN)); } catch (e) { run(false, e); }
        }, false);
    }
);
...
redUnit.testGroup(
    ...
    (runner) => {
        ...
        runner.defineTest('Failure: 150 in [0, 100]', (run) => {
            try { RedGPU.RuntimeChecker.validateNumberRange(150, 0, 100); run(true); } catch (e) { run(false, e); }
        }, false);
