...
class Geometry extends ResourceBase {
     ...
     */
    constructor(redGPUContext, vertexBuffer, indexBuffer) {
        super(redGPUContext);
    ...
    #updateVertexBuffer(vertexBuffer) {
        ...
        if (prevBuffer)
            prevBuffer.__removeDirtyPipelineListener(this.#updateVertexBufferState);
        if (vertexBuffer)
            vertexBuffer.__addDirtyPipelineListener(this.#updateVertexBufferState);
        this.#volume = null;
    }
    /**
     * 인덱스 버퍼를 갱신합니다.
    ...
    #updateIndexBuffer(indexBuffer) {
        ...
        if (prevBuffer)
            prevBuffer.__removeDirtyPipelineListener(this.#updateIndexBufferState);
        if (indexBuffer)
            indexBuffer.__addDirtyPipelineListener(this.#updateIndexBufferState);
    }
    /**
