...
class LightManager {
     ...
     * @private
     */
    #limitDirectionalLightCount = 3;
    /**
     * 클러스터 조명(Point + Spot)의 최대 허용 개수입니다.
     * PassClustersLightHelper.MAX_CLUSTER_LIGHTS 값을 사용합니다.
     * @private
     */
    #limitClusterLightCount = PassClustersLightHelper.MAX_CLUSTER_LIGHTS;
    /**
     * 등록된 방향성 조명 목록입니다.
     * @private
     */
     ...
     */
    get limitDirectionalLightCount() {
    ...
    set ambientLight(value) {
        if (!(value instanceof AmbientLight))
            consoleAndThrowError('allow only AmbientLight instance');
     ...
     */
    addSpotLight(value) {
        if (!(value instanceof SpotLight))
            consoleAndThrowError('allow only SpotLight instance');
    ...
    addPointLight(value) {
        if (!(value instanceof PointLight))
            consoleAndThrowError('allow only PointLight instance');
        const isOverLimit = this.#spotLights.length + this.#pointLights.length > this.#limitClusterLightCount;
        if (isOverLimit) {
            consoleAndThrowError('Cannot add more cluster lights. The limit has been reached.');
        }
     ...
     */
    addDirectionalLight(value) {
        if (!(value instanceof DirectionalLight))
            consoleAndThrowError('allow only DirectionalLight instance');
    ...
    #getMainDirectionalLightViewMatrix(view) {
        ...
        const lightPosition = view.scene.lightManager.directionalLights.length ? vec3.fromValues(-view.scene.lightManager.directionalLights[0].direction[0] * distance, -view.scene.lightManager.directionalLights[0].direction[1] * distance, -view.scene.lightManager.directionalLights[0].direction[2] * distance) : vec3.create();
