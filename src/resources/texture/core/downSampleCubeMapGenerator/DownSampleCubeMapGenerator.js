...
class DownSampleCubeMapGenerator {
    #redGPUContext;
    #cubemapComputePipelines = new Map();
    #cubemapBindGroupLayouts = new Map();
    #cubemapShaderModule = null;
    #cubemapUniformBuffer = null;
    #cubemapUniformStructInfo = null;
    #cubemapSampler = null;
    ...
    createSourceTextureView(sourceCubemap, sourceMipLevel) {
    ...
    createTargetTextureView(targetCubemap, targetMipLevel) {
    ...
    createBindGroup(bindGroupLayout, sourceView, targetView) {
     ...
     */
    async downsampleCubemap(sourceCubemap, targetSize = 256, format = 'rgba8unorm') {
        try {
            // 캐시 초기화
            ...
            for (let mipLevel = 0; mipLevel < targetMipLevels; mipLevel++) {
                ...
                await this.#downsampleCubemapMipLevel(sourceCubemap, targetCubemap, sourceMipLevel, mipLevel, currentTargetSize, format);
            }
     ...
     */
    #getCubemapComputePipeline(format) {
        if (!this.#cubemapComputePipelines.has(format)) {
            const { gpuDevice, resourceManager } = this.#redGPUContext;
            // 포맷별 바인드 그룹 레이아웃 생성
            const bindGroupLayout = resourceManager.createBindGroupLayout(`DOWN_SAMPLE_CUBE_GENERATOR_BIND_GROUP_LAYOUT_${format}`, {
                entries: [
                    {
                        binding: 0,
    ...
    #calculateSourceMipLevel(sourceSize, targetSize, targetMipLevel, sourceMipLevels) {
        // 현재 타겟 밉레벨의 실제 크기
    ...
    async #downsampleCubemapMipLevel(sourceCubemap, targetCubemap, sourceMipLevel, targetMipLevel, targetSize, format) {
        const { gpuDevice } = this.#redGPUContext;
        ...
        computePassEncoder.dispatchWorkgroups(Math.ceil(targetSize / this.#COMPUTE_WORKGROUP_SIZE_X), Math.ceil(targetSize / this.#COMPUTE_WORKGROUP_SIZE_Y), 6 // 6개 면
        );
    ...
    #updateCubemapUniforms(sourceMipLevel, targetMipLevel, targetSize) {
        const { gpuDevice } = this.#redGPUContext;
        // 유니폼 데이터 업데이트
        ...
        gpuDevice.queue.writeBuffer(this.#cubemapUniformBuffer, 0, uniformData);
    }
}
