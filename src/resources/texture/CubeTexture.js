...
class CubeTexture extends ManagementResourceBase {
     ...
     */
    constructor(redGPUContext, srcList, useMipMap = true, onLoad, onError, format) {
    ...
    set srcList(value) {
        ...
        if (this.#srcList?.length)
            this.#loadBitmapTexture(this.#srcList);
    }
    /** 밉맵 사용 여부 반환 */
    get useMipmap() {
     ...
     * @category Texture
     */
    setGPUTextureDirectly(gpuTexture, cacheKey, useMipmap = true) {
        // 기존 텍스처 정리
        if (this.#gpuTexture) {
    ...
    #createGPUTexture() {
        ...
        this.#mipLevelCount = 1;
        {
            ...
            this.targetResourceManagedState.videoMemory += this.#videoMemorySize;
            if (this.#useMipmap)
                mipmapGenerator.generateMipmap(newGPUTexture, textureDescriptor);
     ...
     * @category Texture
     */
    async #loadBitmapTexture(srcList) {
 ...
 * @category Texture
 */
async function loadAndCreateBitmapImages(srcList) {
