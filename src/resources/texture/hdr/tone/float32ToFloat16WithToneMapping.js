 ...
 */
export async function float32ToFloat16WithToneMapping(redGPUContext, float32Data, options) {
    ...
    try {
        ...
        const { computePipeline, bindGroup } = createPipelineAndBindGroup(gpuDevice, computeShader, buffers);
        // 🔧 컴퓨트 패스 실행
        const result = await executeCompute(gpuDevice, computePipeline, bindGroup, buffers.outputBuffer, buffers.readBuffer, width, height, workgroupSize, pixelCount);
        // 🗑️ 버퍼 정리
 ...
 */
function createFloat16ToneMappingShaderCode(workgroupSize) {
    return `
struct Constants {
    exposure: f32,
    width: u32,
...
function createBuffers(gpuDevice, float32Data, pixelCount) {
 ...
 * 🔧 상수 데이터 업로드
 */
function uploadConstants(gpuDevice, constantsBuffer, exposure, width, height) {
 ...
 * 🔧 파이프라인 및 바인드 그룹 생성
 */
function createPipelineAndBindGroup(gpuDevice, computeShader, buffers) {
 ...
 * 🔧 컴퓨트 패스 실행
 */
async function executeCompute(gpuDevice, computePipeline, bindGroup, outputBuffer, readBuffer, width, height, workgroupSize, pixelCount) {
    ...
    computePass.end();
    // 🔧 결과를 읽기용 버퍼로 복사
    commandEncoder.copyBufferToBuffer(outputBuffer, 0, readBuffer, 0, pixelCount * 8 // Float16 RGBA = 8 bytes per pixel
    );
 ...
 * 🗑️ 버퍼 정리
 */
function cleanupBuffers(buffers) {
