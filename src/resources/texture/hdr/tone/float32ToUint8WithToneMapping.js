 ...
 */
export async function float32ToUint8WithToneMapping(redGPUContext, float32Data, options) {
    ...
    try {
        ...
        // 🔧 컴퓨트 패스 실행
        const result = await executeCompute(gpuDevice, computePipeline, bindGroup, buffers.outputBuffer, // outputBuffer를 직접 전달
        buffers.readBuffer, width, height, workgroupSize, pixelCount);
        // 🗑️ 버퍼 정리
 ...
 * 🎨 톤매핑 컴퓨트 셰이더 코드 생성
 */
function createToneMappingShaderCode(workgroupSize) {
    return `
        struct Constants {
            exposure: f32,
            width: u32,
 ...
 * 🔧 컴퓨트 패스 실행
 */
async function executeCompute(gpuDevice, computePipeline, bindGroup, outputBuffer, // outputBuffer를 직접 받음
readBuffer, width, height, workgroupSize, pixelCount) {
