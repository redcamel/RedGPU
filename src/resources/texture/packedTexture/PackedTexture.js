 ...
 */
const instanceMappingKeys = new WeakMap();
let globalPipeline;
let globalBindGroupLayout;
let mappingBuffer;
/**
...
class PackedTexture {
     ...
     */
    async packing(textures, width, height, label, componentMapping) {
    ...
    #initializeGlobals() {
        ...
        if (!globalBindGroupLayout) {
            globalBindGroupLayout = this.#redGPUContext.resourceManager.createBindGroupLayout('PACK_TEXTURE_BIND_GROUP_LAYOUT', {
                entries: [
     ...
     */
    #handleCacheManagement(mappingKey) {
     ...
     * @param mappingKey - 매핑 키 문자열
     */
    async #createPackedTexture(textures, width, height, label, mapping, mappingKey) {
     ...
     */
    #executeRenderPass(packedTexture) {
        const { resourceManager } = this.#redGPUContext;
