...
class AUniformBaseBuffer extends ABaseBuffer {
    ...
    writeBuffers(targetList) {
        const { gpuDevice } = this.redGPUContext;
        let i = targetList.length;
        while (i--) {
            const [target, value] = targetList[i];
            gpuDevice.queue.writeBuffer(this.gpuBuffer, target.uniformOffset, new target.View(typeof value === "number" ? [value] : value));
        }
    }
    writeBuffer(target, value) {
