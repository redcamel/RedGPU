...
const getGLTFBuffersResources = (gltfLoader, gltfData, callback) => {
    ...
    buffers.forEach((buffer, bufferIndex) => {
        ...
        if (buffer.uri instanceof ArrayBuffer) {
            ...
            processNonArrayBuffer(buffer.uri);
        function processArrayBuffer(uriDataView) {
            loadedBuffers++;
            resultContainer[bufferIndex] = uriDataView;
            checkLoadingStatus();
        }
        function processNonArrayBuffer(uri) {
