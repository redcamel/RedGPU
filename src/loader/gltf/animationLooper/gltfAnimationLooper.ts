...
// 이진 탐색 헬퍼 함수
const binarySearchTimeFrame = (
    timeList: Float32Array,
    currentTime: number,
    listLength: number,
    lastPrevIdx: number = 0
): [number, number] => {
    // 최적화: 대부분의 경우 이전 프레임과 같거나 +1
    if (lastPrevIdx < listLength - 1) {
        if (timeList[lastPrevIdx] <= currentTime && currentTime < timeList[lastPrevIdx + 1]) {
            return [lastPrevIdx, lastPrevIdx + 1];
        }
        if (lastPrevIdx + 1 < listLength - 1 &&
            timeList[lastPrevIdx + 1] <= currentTime &&
            currentTime < timeList[lastPrevIdx + 2]) {
            return [lastPrevIdx + 1, lastPrevIdx + 2];
        }
    }

    // 엣지 케이스: 시작 부분
    if (currentTime <= timeList[0]) {
        return [listLength - 1, 0];
    }

    // 엣지 케이스: 끝 부분
    if (currentTime >= timeList[listLength - 1]) {
        return [listLength - 1, 0];
    }
    ...
    while (left < right - 1) {
        const mid = (left + right) >> 1; // 비트 시프트로 나누기 2
        if (timeList[mid] <= currentTime) {
            left = mid;
        } else {
            right = mid;
        }
...
// NaN 체크 최적화 (toString() 대신 빠른 체크)
const fixNaN = (val: number): number => val === val ? val : 0;
...
const gltfAnimationLooper = (
    ...
    while (playAnimationInfoIDX--) {
        ...
        while (targetAniTrackIDX--) {
            ...
            [previousTimeDataIDX, nextTimeDataIDX] = binarySearchTimeFrame(
                targetTimeDataList,
                currentTime,
            ...
            if (isCUBICSPLINE) {
                interpolationValue = fixNaN(nextTimeFrame - previousTimeFrame);
                p = fixNaN((currentTime - previousTimeFrame) / interpolationValue);
                pp = p * p;
                ppp = pp * p;
                s2 = -2 * ppp + 3 * pp;
                s3 = ppp - pp;
                ...
                if (interpolation == 'STEP') {
                    ...
                    interpolationValue = fixNaN((currentTime - previousTimeFrame) / (nextTimeFrame - previousTimeFrame));
                }
            }
