...
const parseMaterialTexture = (gltfLoader, tMaterial, textureInfo, targetTextureKey, format, useMipmap = true) => {
...
const getURL = function (gltfLoader, scenesData, sourceGlTfId) {
...
const getSamplerInfo = function (gltfLoader, scenesData, samplerGlTfId) {
    const result = {
    ...
    if (scenesData.samplers) {
        ...
        if (t0) {
            if ('magFilter' in t0)
                result.magFilter = magFilterTable[t0.magFilter] || GPU_MIPMAP_FILTER_MODE.LINEAR;
            if ('minFilter' in t0)
                result.minFilter = minFilterTable[t0.minFilter] || GPU_MIPMAP_FILTER_MODE.LINEAR;
            if ('wrapS' in t0)
                result.addressModeU = wrapTable[t0.wrapS];
            if ('wrapT' in t0)
                result.addressModeV = wrapTable[t0.wrapT];
        }
    }
    else {
    }
