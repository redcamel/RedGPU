 ...
 */
const parseMaterialInfo_GLTF = (gltfLoader, gltfData, meshPrimitive) => {
    ...
    if ('material' in meshPrimitive) {
        ...
        const { pbrMetallicRoughness: pbrMetallicRoughnessInfo, normalTexture: normalTextureInfo, emissiveTexture: emissiveTextureInfo, occlusionTexture: occlusionTextureInfo } = materialInfo;
        currentMaterial.emissiveFactor = materialInfo.emissiveFactor || [0, 0, 0];
        if (pbrMetallicRoughnessInfo) {
            const { metallicRoughnessTexture: metallicRoughnessTextureInfo, baseColorTexture: baseColorTextureInfo, } = pbrMetallicRoughnessInfo;
            ...
            if (baseColorTextureInfo) {
                parseMaterialTexture(gltfLoader, currentMaterial, baseColorTextureInfo, 'baseColorTexture', `${navigator.gpu.getPreferredCanvasFormat()}-srgb`);
            }
            if (metallicRoughnessTextureInfo) {
                parseMaterialTexture(gltfLoader, currentMaterial, metallicRoughnessTextureInfo, 'metallicRoughnessTexture', null, false);
            }
        }
        if (normalTextureInfo) {
            parseMaterialTexture(gltfLoader, currentMaterial, normalTextureInfo, 'normalTexture');
            const { scale } = normalTextureInfo;
            currentMaterial.normalScale = scale != undefined ? scale : 1;
        }
        if (emissiveTextureInfo) {
            parseMaterialTexture(gltfLoader, currentMaterial, emissiveTextureInfo, 'emissiveTexture', `${navigator.gpu.getPreferredCanvasFormat()}-srgb`);
        }
        if (occlusionTextureInfo) {
            parseMaterialTexture(gltfLoader, currentMaterial, occlusionTextureInfo, 'occlusionTexture', null, false);
        ...
        if ('extensions' in materialInfo) {
            ...
            const { KHR_materials_clearcoat, KHR_materials_emissive_strength, KHR_materials_transmission, KHR_materials_diffuse_transmission, KHR_materials_volume, KHR_materials_unlit, KHR_materials_ior, KHR_materials_sheen, KHR_materials_specular, KHR_materials_dispersion, KHR_materials_anisotropy, KHR_materials_iridescence } = extensions;
