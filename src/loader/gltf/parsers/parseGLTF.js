...
const parseGLTF = (gltfLoader, gltfData, callBack) => {
    //  작업을 여러 프레임에 분산
                ...
                requestAnimationFrame(() => {
                    parseScene_GLTF(gltfLoader, gltfData, () => {
                        requestAnimationFrame(() => {
                            ...
                            // TextureLoader 실행
                            new TextureLoader(gltfLoader.redGPUContext, Object.values(gltfLoader.parsingResult.textureRawList), result => {
                                result.textures.forEach((textureLoaderData) => {
                                    const { targetTextureKey, targetSamplerKey, samplerList } = textureLoaderData.srcInfo;
                                    textureLoaderData.srcInfo.materialList.forEach((material, index) => {
