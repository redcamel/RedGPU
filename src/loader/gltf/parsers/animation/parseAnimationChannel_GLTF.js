 ...
 */
const parseAnimationChannel_GLTF = (gltfLoader, parsedSingleClip, animationChannel, animationSamplers, gltfData) => {
    let tMesh;
    let aniTrack; //
    ...
    if ('mesh' in tNode) {
        ...
        if (gltfLoader.parsingResult.groups[channelTargetDataNodeGlTfId]) {
            ...
            return console.log('여기로 오는경우가 있는건가2');
    }
    if (channelTargetDataPath == 'scale'
        ...
        aniTrack = new AniTrack_GLTF(channelTargetDataPath, new parseAnimationInfo_GLTF(gltfLoader, gltfData, tSampler.input), new parseAnimationInfo_GLTF(gltfLoader, gltfData, tSampler.output), tSampler.interpolation, tMesh, targetMeshes);
    ...
    if (aniTrack) {
        ...
        if (parsedSingleClip.minTime > dataList[0])
            parsedSingleClip.minTime = dataList[0];
        if (parsedSingleClip.maxTime < dataList[dataList.length - 1])
            parsedSingleClip.maxTime = dataList[dataList.length - 1];
    }
};
