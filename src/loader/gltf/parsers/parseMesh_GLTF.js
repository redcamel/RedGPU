...
const parseMesh_GLTF = function (gltfLoader, gltfData, gltfMesh, nodeGlTfId) {
    ...
    for (i; i < primitivesLength; i++) {
        const meshPrimitive = primitives[i];
        let tMesh;
        let tMaterial;
        let indices = [];
        // 어트리뷰트에서 파싱되는놈들
        ...
        if (attributes) {
            for (const key in attributes) {
                ...
                // 어트리뷰트 갈궈서 파악함
                parseAttributeInfo_GLTF(key, accessorInfo, vertices, uvs, uvs1, uvs2, normals, jointWeights, joints, verticesColor_0, tangents);
                // 스파스 정보도 갈굼
                if (accessorInfo.accessor.sparse) {
                    parseSparse_GLTF(gltfLoader, key, accessorInfo.accessor, gltfData, vertices);
                }
            }
        }
        // 인덱스 파싱
        ...
        if (verticesColor_0.length)
            tMaterial.useVertexColor_0 = true;
        if (tangents.length)
        ...
        if (vertices.length)
            tInterleaveInfoList['aVertexPosition'] = VertexInterleaveType.float32x3;
        if (normalData.length)
            tInterleaveInfoList['aVertexNormal'] = VertexInterleaveType.float32x3;
        if (uvs.length)
            tInterleaveInfoList['aTexcoord'] = VertexInterleaveType.float32x2;
        if (uvs2.length)
            tInterleaveInfoList['aTexcoord1'] = VertexInterleaveType.float32x2;
        else if (uvs1.length)
            tInterleaveInfoList['aTexcoord1'] = VertexInterleaveType.float32x2;
        else if (uvs.length)
            tInterleaveInfoList['aTexcoord1'] = VertexInterleaveType.float32x2;
        ...
        const weightBuffer = new VertexBuffer(redGPUContext, weightData, new VertexInterleavedStruct({
            aVertexWeight: VertexInterleaveType.float32x4,
        }), undefined, `Weight_${gltfLoader.url}_${nodeGlTfId}_${i}`);
        const jointBuffer = new IndexBuffer(redGPUContext, jointData, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST, `Joint_${gltfLoader.url}_${nodeGlTfId}_${i}`);
        /////////////////////////////////////////////////////////
        ...
        tGeo = new Geometry(redGPUContext, hasVertexBuffer || new VertexBuffer(redGPUContext, interleaveData, new VertexInterleavedStruct(tInterleaveInfoList), undefined, vertexCacheKey), !noIndexBuffer && indices.length ? new IndexBuffer(redGPUContext, new Uint32Array(indices), undefined, `Index_${gltfLoader.url}_${nodeGlTfId}_${i}`) : null);
        ...
        if (tDrawMode)
            tMesh.primitiveState.topology = tDrawMode;
        else
            tMesh.primitiveState.topology = GPU_PRIMITIVE_TOPOLOGY.TRIANGLE_LIST;
        //
        ...
        if (!tMesh.gpuRenderInfo)
            tMesh.initGPURenderInfos();
        let NUM = 0;
        {
