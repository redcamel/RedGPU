...
const parseFileGLB = async (gltfLoader, callBack) => {
    ...
    const promise = new Promise((resolve, reject) => {
        getArrayBufferFromSrc(loadFilePath, (buffer) => {
...
const parseArrayBuffer = async (gltfLoader, buffer, callBack) => {
    ...
    for (let chunkIndex = 0; chunkIndex < byteLength;) {
        ...
        switch (chunkType) {
            case BINPACKER_CHUNK_TYPE_JSON:
                const contentArray = new Uint8Array(buffer, BINPACKER_HEADER_LENGTH + chunkIndex, chunkLength);
 ...
 */
const processImagesIfExist = (gltfData, binaryChunk) => {
