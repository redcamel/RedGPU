...
class ParsedSkinInfo_GLTF {
    ...
    createCompute(redGPUContext, device, vertexStorageBuffer, weightBuffer, jointBuffer) {
        const source = `
			struct Uniforms {
    ...
    `;
        ...
        device.queue.writeBuffer(this.uniformBuffer, this.jointData.byteLength, new Float32Array(this.inverseBindMatrices.map(v => Array.from(v)).flat()));
        ...
        device.queue.writeBuffer(this.uniformBuffer, this.jointData.byteLength + (this.joints.length * 16 * 4), searchJointIndexTableData);
        // keepLog(
        // 	this.usedJoints,
