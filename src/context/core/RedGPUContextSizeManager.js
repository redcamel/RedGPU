...
class RedGPUContextSizeManager {
     ...
     * @param value
     */
    static validateSizeValue = (value) => {
        switch (typeof value) {
            case 'number':
            ...
            case 'string':
                ...
                if (!sizeModel.test(value))
                    consoleAndThrowError(`allow positive number, %, px model / input : ${value}`);
                break;
            default:
     ...
     * @param value
     */
    static validatePositionValue = (value) => {
        ...
        switch (typeof value) {
            ...
            case 'string':
                ...
                if (!sizeModel.test(value))
                    consoleAndThrowError(`allow number, %, px model  / input : ${value}`);
                break;
            default:
    ...
    static getPixelDimension(parentRect, key, value) {
     ...
     */
    static calculateSizeFromString(rect, key, value) {
        if (value.includes('%')) {
    ...
    setSize(w = this.#width, h = this.#height) {
        ...
        console.log(`${this.constructor.name}.setSize - input : ${w},${h} | output : ${tW}, ${tH}`, {
            width: this.#width,
            height: this.#height,
            pixelRectArray: this.#pixelRectArray
        });
    }
    #updatePixelRect(tW, tH) {
    ...
    #updateViewsSize() {
        ...
        this.#redGPUContext.viewList.forEach((view) => {
            view.setSize();
            view.setPosition();
        });
    }
     ...
     * @return {void}
     */
    #changeCanvasStyles(width, height) {
