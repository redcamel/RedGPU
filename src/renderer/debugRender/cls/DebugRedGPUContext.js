import { createDebugTitle, getDebugFormatValue, makeBooleanDebug, makeColorDebug, updateDebugItemValue } from "../core/debugFunc";
...
class DebugStatisticsDomService extends ADebugStatisticsDomService {
    ...
    #generateDebugItemsHtml() {
        ...
        const stats = [
            ...
            ...debugStats.map(stat => this.#generateDebugItemHtml(stat)),
    ...
    #updateDebugStat(redGPUContext, stat) {
        const isAntialiasing = ['useMSAA', 'useFXAA', 'useTAA'].includes(stat);
        const value = isAntialiasing ? redGPUContext.antialiasingManager[stat] : redGPUContext[stat];
        updateDebugItemValue(this.dom, `redGPUContext_${stat}`, isAntialiasing ? makeBooleanDebug(stat, value) : getDebugFormatValue(value));
    }
    #updateViewportSize(redGPUContext) {
