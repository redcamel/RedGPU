...
class DrawBufferManager {
	...
	#createCompute() {
		...
		this.#computeShaderModule = gpuDevice.createShaderModule({
			label: 'DrawBuffer Culling Compute',
			code: SHADER_INFO.defaultSource
		})
		this.#computePipeline = gpuDevice.createComputePipeline({
			label: 'DrawBuffer Culling Compute Pipeline',
			layout: 'auto',
			compute: {
		...
		this.#uniformBuffer = gpuDevice.createBuffer({
			size: SHADER_INFO.uniforms.cameraFrustum.arrayBufferByteLength,
			usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST ,
			label: 'DrawBuffer Culling Uniform Buffer'
		})
		{
			const entries = [{
				binding: 0,
			...
			this.#bufferPool.forEach((buffer,index)=>{
				entries.push({
					binding: index+1,
					resource: {
						buffer: buffer,
	...
	render() {
		...
		const passEncoder = commandEncoder.beginComputePass(
			{
				label: 'DrawBuffer Culling Compute Pass',
			}
		);
		passEncoder.setPipeline(this.#computePipeline);
		passEncoder.setBindGroup(0, this.#computeBindGroup);
		const workgroupSize = 64
		const totalCommands = this.getTotalCommandCount()
		const numWorkgroups = Math.ceil(totalCommands / workgroupSize)
