...
import { getFragmentBindGroupLayoutDescriptorFromShaderInfo, getVertexBindGroupLayoutDescriptorFromShaderInfo } from "../../material/core";
...
class FinalRender {
     ...
     */
    render(redGPUContext, viewList_renderPassDescriptorList) {
        ...
        if (canvasW === 0 || canvasH === 0)
            return;
        //
        const finalRenderPassDesc = this.#getFinalRenderPassDesc(redGPUContext);
        const finalRenderCommandEnc = gpuDevice.createCommandEncoder();
        const finalRenderPassEnc = finalRenderCommandEnc.beginRenderPass(finalRenderPassDesc);
        finalRenderPassEnc.setViewport(0, 0, canvasW, canvasH, 0, 1);
        finalRenderPassEnc.setScissorRect(0, 0, canvasW, canvasH);
        //
        if (!this.#vertexBindGroupLayout || changedMSAA)
            this.#initGPUDetails(redGPUContext);
        this.#renderViewList(redGPUContext, finalRenderPassEnc, viewList_renderPassDescriptorList.map((v) => {
            const temp = v.colorAttachments[0];
            return temp.postEffectView || temp.pickingView || temp.resolveTarget || temp.view;
        }), canvasW, canvasH, useMSAA);
        finalRenderPassEnc.end();
        gpuDevice.queue.submit([finalRenderCommandEnc.finish()]);
    }
    #updateFinalViewBackgroundColor(view, index) {
        {
    ...
    #renderViewList(redGPUContext, finalRenderPassEnc, resultTextureViews, canvasW, canvasH, useMSAA) {
        const { gpuDevice } = redGPUContext;
        resultTextureViews.forEach((gpuTextureView, index) => {
            ...
            const vertexUniformBuffer = this.#vertexUniformBuffers[index];
            const vertexUniformBindGroup = this.#vertexUniformBindGroups[index];
            gpuDevice.queue.writeBuffer(vertexUniformBuffer.gpuBuffer, VERTEX_UNIFORM_STRUCT.members.modelMatrix.uniformOffset, 
            // new VERTEX_UNIFORM_STRUCT.members.modelMatrix.View(projectionMatrix),
            projectionMatrix);
            //
            const needNewBindGroup = redGPUContext.antialiasingManager.changedMSAA
     ...
     */
    #initGPUDetails(redGPUContext) {
        const { resourceManager, } = redGPUContext;
        this.#vertexBindGroupLayout = resourceManager.createBindGroupLayout('FINAL_RENDER_VERTEX_BIND_GROUP_LAYOUT', getVertexBindGroupLayoutDescriptorFromShaderInfo(VERTEX_SHADER_INFO, 0));
        ...
        this.#fragmentBindGroupLayout = resourceManager.createBindGroupLayout('FINAL_RENDER_BIND_GROUP_LAYOUT', getFragmentBindGroupLayoutDescriptorFromShaderInfo(FRAGMENT_SHADER_INFO, 1));
     ...
     */
    #validateViewGpuDetails(redGPUContext, index) {
        const { gpuDevice } = redGPUContext;
        if (!this.#vertexUniformBuffers[index]) {
            const vertexUniformData = new ArrayBuffer(VERTEX_UNIFORM_STRUCT.arrayBufferByteLength);
            const tVUniformBuffer = this.#vertexUniformBuffers[index] = new UniformBuffer(redGPUContext, vertexUniformData, `FinalRender_View(${index})_VertexUniform`);
            const vertexBindGroupDesc = {
    ...
    #getFinalRenderPassDesc(redGPUContext) {
