...
class Renderer {
    ...
    renderFrame(redGPUContext, time) {
        if (!this.#finalRender)
            this.#finalRender = new FinalRender();
        if (!this.#debugRender)
            this.#debugRender = new DebugRender(redGPUContext);
        // 오브젝트 렌더시작
        const viewList_renderPassDescriptorList = [];
        {
            let i = 0;
    ...
    #renderPassViewBasicLayer(view, commandEncoder, renderPassDescriptor) {
        ...
        if (axis)
            axis.render(renderViewStateData);
        if (grid)
            grid.render(renderViewStateData);
    ...
    #batchUpdateSkinMatrices(redGPUContext, renderViewStateData) {
        ...
        for (let i = 0; i < skinListNum; i++) {
            ...
            if (!skinInfo.computeShader) {
                skinInfo.createCompute(redGPUContext, gpuDevice, mesh.animationInfo.skinInfo.vertexStorageBuffer, mesh.animationInfo.weightBuffer, mesh.animationInfo.jointBuffer);
            }
            {
    ...
    #createAttachmentsForView(view) {
        const { scene, redGPUContext, viewRenderTextureManager } = view;
        const { depthTextureView, gBufferColorTextureView, gBufferColorResolveTextureView, gBufferNormalTextureView, gBufferNormalResolveTextureView, gBufferMotionVectorTextureView, gBufferMotionVectorResolveTextureView, } = viewRenderTextureManager;
        const { antialiasingManager } = redGPUContext;
        const { useMSAA } = antialiasingManager;
        ...
        const gBufferNormalTextureAttachment = {
            view: gBufferNormalTextureView,
            clearValue: { r: 0, g: 0, b: 0, a: 0 },
            loadOp: GPU_LOAD_OP.CLEAR,
            storeOp: GPU_STORE_OP.STORE
        };
        const gBufferMotionVectorTextureAttachment = {
