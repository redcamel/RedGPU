...
fn main(inputData:InputData) -> OutputFragment {
    ...
    // Î™®Îì† ?çÏä§Ï≤??Ä?ÖÏóê ?Ä??UV Ï¢åÌëúÎ•?Ï¥àÍ∏∞??

    // Í∏∞Î≥∏ PBR ?çÏä§Ï≤?UV Ï¢åÌëú
    ...
    // [KO] ?¥Î¶¨?¥ÏΩî???±ÏùÑ ?ÑÌï¥ Î≥Ä????Í∏∞Ìïò Î≤ïÏÑ†??Î≥¥Ï°¥?©Îãà??
    ...
    #redgpu_if useKHR_materials_clearcoat
    {
        if(clearcoatParameter != 0.0){
            ...
            #redgpu_if useKHR_clearcoatNormalTexture
            {
                ...
                // [KO] ?¥Î¶¨?¥ÏΩî??TBN?Ä Î≥Ä?ïÎêú N???ÑÎãå Í∏∞Ìïò Î≤ïÏÑ†(geometricNormal)??Í∏∞Ï??ºÎ°ú Íµ¨Ï∂ï?¥Ïïº ?©Îãà??
    ...
    #redgpu_if useKHR_materials_anisotropy
    {
       ...
       // ?çÏä§Ï≤òÏóê???ΩÏñ¥??Î∞©Ìñ•???†Îãà???åÏ†Ñ?âÏùÑ Í≤∞Ìï©
       anisotropicDirection = rotationMtx * anisotropicDirection;

       // 3. ÏµúÏ¢Ö ?¥Î∞©???ÑÏ††??Î∞îÏù¥?ÑÏ††??Í≥ÑÏÇ∞ (TBN Í≥µÍ∞Ñ?ºÎ°ú ?¨ÏòÅ)
    ...
    #redgpu_if useKHR_materials_transmission
        transmissionRefraction = getTransmissionRefraction(
            ...
            systemUniforms.projection.projectionViewMatrix, input_vertexPosition, input_ndcPosition,
            V, N,
            renderPath1ResultTexture, renderPath1ResultTextureSampler
        );
    #redgpu_endIf
    // ---------- Í∏∞Î≥∏ F0 Í≥ÑÏÇ∞ ----------
//    let F0_dielectric: vec3<f32> =  vec3(pow((1.0 - ior) / (1.0 + ior), 2.0)) ; // ?†Ï†ÑÏ≤?Î∞òÏÇ¨??
//    let F0_metal = baseColor.rgb; // Í∏àÏÜç Î∞òÏÇ¨??
//    var F0 = mix(F0_dielectric, F0_metal, metallicParameter); // Í∏∞Î≥∏ Î∞òÏÇ¨??
    let F0_dielectric_base = vec3(pow((1.0 - ior) / (1.0 + ior), 2.0));
    // KHR_materials_specular ?ÅÏö©
    var F0_dielectric = F0_dielectric_base *  specularColor;
    var F0_metal = baseColor.rgb; // Í∏àÏÜç Î∞òÏÇ¨??
    ...
    #redgpu_endIf
    let F0 = mix(F0_dielectric, F0_metal, metallicParameter); // Í∏∞Î≥∏ Î∞òÏÇ¨??

    // ---------- ÏßÅÏ†ë Ï°∞Î™Ö Í≥ÑÏÇ∞ - directional ----------
    ...
    if (u_usePrefilterTexture) {
        ...
        // ---------- ibl (BRDF LUT ?òÌîåÎß? ----------
        // NdotV?Ä RoughnessÎ•?Ï¢åÌëúÎ°??¨Ïö©?òÏó¨ ÎØ∏Î¶¨ Í≥ÑÏÇ∞??Scale(x)Í≥?Bias(y) Í∞íÏùÑ Í∞Ä?∏Ïò¥
        let envBRDF = textureSampleLevel(ibl_brdfLUTTexture, prefilterTextureSampler, vec2<f32>(NdotV, roughnessParameter), 0.0).rg;

        // Í±∞Ïπ†Í∏∞Î? Í≥†Î†§??Í∞??åÌä∏Î≥??µÌï© Î∞òÏÇ¨??Í≥ÑÏÇ∞ (Split Sum Approximation)
        ...
        // Transmission ?±Ïóê???¨Ïö©?òÎäî Smith Í∏∞Ìïò Í∞êÏá† Í∑ºÏÇ¨ (Í∏∞Ï°¥ Î°úÏßÅ ?†Ï?)
        ...
        #redgpu_if useKHR_materials_diffuse_transmission
        {
            // ?ÑÎ©¥ ?∞Î????ÑÌïú ?òÌîåÎß?Î∞©Ìñ• (back side)
            ...
            if(length(refractedDir) > 0.0001) {
                ...
                var attenuatedBackground = transmissionRefraction;
                 if (u_useKHR_materials_volume) {
                     ...
                     // ?†Ìö®???âÏÉÅ Î∞?Í±∞Î¶¨ Í∞??ïÎ≥¥ (Î¨ºÎ¶¨?ÅÏúºÎ°??òÎ? ?àÎäî Î≤îÏúÑ)
                     ...
                     // Beer-Lambert Î≤ïÏπô???∞Î•∏ Í∞êÏá† Í≥ÑÏÇ∞
        ...
        // ---------- ibl Í∏∞Î≥∏ ?ºÌï© ----------
        let metallicPart = envIBL_METAL * metallicParameter ; // Í∏àÏÜç ?åÌä∏ Í≥ÑÏÇ∞
        let dielectricPart = envIBL_DIELECTRIC * (1.0 - metallicParameter) ; // ?†Ï†ÑÏ≤??åÌä∏ Í≥ÑÏÇ∞
        var indirectLighting = (metallicPart + dielectricPart) * sheenAlbedoScaling + sheenIBLContribution; // sheen ?åÌä∏ ?¨Ìï®
        // ---------- ibl clearcoat Í≥ÑÏÇ∞ ----------
        #redgpu_if useKHR_materials_clearcoat
            if (clearcoatParameter > 0.0) {
                 // ?¥Î¶¨?¥ÏΩî??Î∞òÏÇ¨ Î≤°ÌÑ∞?Ä Î∑?Í∞ÅÎèÑ Í≥ÑÏÇ∞
                 ...
                 // ?¥Î¶¨?¥ÏΩî?∏Îäî Î≥¥ÌÜµ Íµ¥Ï†àÎ•?1.5(F0 = 0.04) Í≥†Ï†ï?ºÎ°ú Ï∑®Í∏â??
                 ...
                 //   ÏµúÏ¢Ö ?©ÏÇ∞ (?êÎÑàÏßÄ Î≥¥Ï°¥: ?¥Î¶¨?¥ÏΩî?∏Í? Î∞òÏÇ¨??ÎßåÌÅº ?òÏúÑ ?àÏù¥??Ï°∞Î™Ö?Ä Í∞êÏá†??
        ...
        // IBL ?çÏä§Ï≤??ÜÎäî Í≤ΩÏö∞ ?®Ïàú Ï£ºÎ?Í¥??¨Ïö©
...
//  // ?îÎ≤ÑÍπ? Î™®ÏÖòÎ≤°ÌÑ∞ Ï¶ùÌè≠?òÏó¨ ?ïÏù∏
//   let amplifiedMotion = inputData.motionVector * 50.0;  // 50Î∞?Ï¶ùÌè≠
...
fn calcLight(
    ...
    // Ï°∞Î™Ö Í≥ÑÏÇ∞Í≤∞Í≥º
    // Í∏àÏÜçÍ≥??†Ï†ÑÏ≤?Î∂ÄÎ∂ÑÏùÑ Í∞ÅÍ∞Å Í≥ÑÏÇ∞
...
//    let dielectricPart = DIELECTRIC_BRDF * (1 - metallicParameter) * sheen_albedo_scaling ;
    ...
    #redgpu_else
        // ?¨Í≥ºÍ∞Ä ?ÜÎäî Í≤ΩÏö∞, Í∏∞Ï°¥Ï≤òÎüº ?ëÏàò Í∞íÎßå ?àÏö©
