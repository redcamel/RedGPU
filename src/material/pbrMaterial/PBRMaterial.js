...
const parseExtensionShaderCode = (source) => {
...
class PBRMaterial extends ABitmapBaseMaterial {
    ...
    constructor(redGPUContext) {
        super(redGPUContext, 'PBR_MATERIAL', SHADER_INFO, 2);
    ...
    async setupPackORMTexture() {
        const width = Math.max(this.occlusionTexture?.gpuTexture?.width || 1, this.metallicRoughnessTexture?.gpuTexture?.width || 1);
        const height = Math.max(this.occlusionTexture?.gpuTexture?.height || 1, this.metallicRoughnessTexture?.gpuTexture?.height || 1);
        ...
        }, width, height, 'packedORMTexture');
    }
    async setupPackedKHR_clearcoatTexture_transmission() {
        // Clearcoat + Transmission을 하나의 텍스처로 패킹
        const clearcoatWidth = Math.max(this.KHR_clearcoatTexture?.gpuTexture?.width || 1, this.KHR_clearcoatRoughnessTexture?.gpuTexture?.width || 1);
        const clearcoatHeight = Math.max(this.KHR_clearcoatTexture?.gpuTexture?.height || 1, this.KHR_clearcoatRoughnessTexture?.gpuTexture?.height || 1);
        const transmissionWidth = Math.max(this.KHR_transmissionTexture?.gpuTexture?.width || 1, this.KHR_thicknessTexture?.gpuTexture?.width || 1);
        const transmissionHeight = Math.max(this.KHR_transmissionTexture?.gpuTexture?.height || 1, this.KHR_thicknessTexture?.gpuTexture?.height || 1);
        ...
        }, finalWidth, finalHeight, 'packedKHR_clearcoatTexture_transmission', {
            b: 'r', a: 'g'
        });
    }
    ...
    async setupPackedKHR_diffuse_transmission() {
        const width = Math.max(this.KHR_diffuseTransmissionColorTexture?.gpuTexture?.width || 1, this.KHR_diffuseTransmissionTexture?.gpuTexture?.width || 1);
        const height = Math.max(this.KHR_diffuseTransmissionColorTexture?.gpuTexture?.height || 1, this.KHR_diffuseTransmissionTexture?.gpuTexture?.height || 1);
        ...
        }, width, height, 'packedKHR_diffuse_transmission');
    }
    async setupPackedKHR_sheen() {
        const width = Math.max(this.KHR_sheenColorTexture?.gpuTexture?.width || 1, this.KHR_sheenRoughnessTexture?.gpuTexture?.width || 1);
        const height = Math.max(this.KHR_sheenColorTexture?.gpuTexture?.height || 1, this.KHR_sheenRoughnessTexture?.gpuTexture?.height || 1);
        ...
        }, width, height, 'packedKHR_sheen');
    }
    async setupPackedKHR_iridescence() {
        const width = Math.max(this.KHR_iridescenceTexture?.gpuTexture?.width || 1, this.KHR_iridescenceThicknessTexture?.gpuTexture?.width || 1);
        const height = Math.max(this.KHR_iridescenceTexture?.gpuTexture?.height || 1, this.KHR_iridescenceThicknessTexture?.gpuTexture?.height || 1);
        ...
        }, width, height, 'packedKHR_iridescence');
    }
}
...
const defineTexture = (textureList, useSampler) => {
...
const extensionDefine = (defineList) => {
    defineList.forEach(v => {
        ...
        if (extensionName)
            DefineForFragment.defineBoolean(PBRMaterial, [`use${extensionName}`]);
