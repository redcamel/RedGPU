...
const generateWebGPUCubeMapTextureMipmap = (gpuDevice, texture, face, textureDescriptor) => {
	//TODO 재생성 안되게 처리
	const vShaderModule = gpuDevice.createShaderModule({
		code: vertexSource,
		label: 'vertex_generateWebGPUCubeMapTextureMipmap'
	});
	const fShaderModule = gpuDevice.createShaderModule({
		code: fragmentSource,
		label: 'fragment_generateWebGPUCubeMapTextureMipmap'
	});
...
export default generateWebGPUCubeMapTextureMipmap;
