 ...
 * @category Texture
 */
const imageBitmapToGPUTexture = (gpuDevice, imageBitmaps, textureDescriptor, usePremultiplyAlpha = true) => {
    const newGPUTexture = gpuDevice.createTexture(textureDescriptor);
    //
    for (let i = 0; i < imageBitmaps.length; i++) {
        const targetImgBitmap = imageBitmaps[i];
        const source = { source: targetImgBitmap };
        const destination = {
        ...
        const copySize = [targetImgBitmap.width, targetImgBitmap.height];
        // 생성한 텍스쳐에 데이터를 밀어넣는다.
        gpuDevice.queue.copyExternalImageToTexture(source, destination, copySize);
    }
