...
/**
 * [KO] 3D 객체의 변환 행렬을 이용해 화면 상의 2D 픽셀 좌표를 계산합니다.
 * [EN] Calculates 2D screen pixel coordinates from a 3D object's transformation matrix.
 *
 * [KO] View3D의 카메라 및 프로젝션 정보를 바탕으로, 모델 변환 행렬을 화면 좌표로 변환합니다.
 * [EN] Converts a model transformation matrix to screen coordinates based on View3D's camera and projection information.
 *
 * * ### Example
 * ```typescript
