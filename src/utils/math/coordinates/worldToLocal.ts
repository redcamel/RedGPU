...
/**
 * [KO] 월드 좌표(x, y, z)를 주어진 변환 행렬(targetMatrix)의 역행렬을 이용해 로컬 좌표로 변환합니다.
 * [EN] Converts world coordinates (x, y, z) to local coordinates using the inverse of the given transformation matrix (targetMatrix).
 *
 * [KO] 입력 좌표(x, y, z)에 대해 targetMatrix의 역행렬을 적용한 결과를 [x, y, z] 배열로 반환합니다. 각 좌표값은 숫자인지 검증하며, 행렬 곱셈 순서는 invert(targetMatrix) * [x, y, z]입니다.
 * [EN] Applies the inverse of targetMatrix to the input coordinates (x, y, z) and returns the result as an [x, y, z] array. Validates that each coordinate is a number. The matrix multiplication order is invert(targetMatrix) * [x, y, z].
 *
