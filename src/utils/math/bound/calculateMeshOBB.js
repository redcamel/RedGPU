...
const calculateMeshOBB = (mesh) => {
    ...
    vec3.transformMat4(transformedCenter, originalCenter, mesh.modelMatrix);
    // 회전과 스케일 매트릭스 추출
    ...
    const transformedHalfExtents = [0, 0, 0];
    ...
    for (let i = 0; i < 3; i++) {
        ...
        // 축 벡터 변환
        vec3.transformMat4(axes[i], originalAxis, rotationScaleMatrix);
        // 변환된 반크기 (벡터의 길이)
