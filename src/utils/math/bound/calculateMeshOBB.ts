...
/**
 * [KO] 메시의 로컬 AABB를 기반으로 메시의 modelMatrix를 적용한 방향성 경계 상자(OBB)를 계산합니다.
 * [EN] Calculates a world-space Oriented Bounding Box (OBB) by applying the mesh's modelMatrix to its local AABB.
 *
 * [KO] 메시의 지오메트리 볼륨 정보를 기반으로 중심점, 반치수, 방향 행렬이 포함된 OBB를 반환합니다.
 * [EN] Returns an OBB containing center, half-extents, and orientation matrix based on the mesh's geometry volume information.
 *
 * * ### Example
 * ```typescript
