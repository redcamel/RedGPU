...
const validateCompareList = Object.values(GPU_COMPARE_FUNCTION);
/**
...
class DepthStencilState {
    ...
    set format(value) {
        if (this.#formatValues.includes(value)) {
            ...
            consoleAndThrowError(`Invalid value for format. Received ${value}. Expected one of: ${this.#formatValues.join(", ")}`);
    }
    /**
     * 깊이 버퍼 쓰기 활성화 여부 반환
     * @category Buffer
    ...
    set depthCompare(value) {
        if (validateCompareList.includes(value)) {
            ...
            consoleAndThrowError(`Invalid value for depthCompare. Received ${value}. Expected one of: ${validateCompareList.join(", ")}`);
    }
    /**
     * 스텐실 전면 상태 반환
     * @category Buffer
