...
class PrimitiveState {
    ...
    set topology(value) {
        if (validatePrimitiveTopology.includes(value)) {
            ...
            consoleAndThrowError(`Invalid value for topology. Received ${value}. Expected one of: ${validatePrimitiveTopology.join(", ")}`);
    }
    /**
     * 스트립 인덱스 포맷 반환
     * @category Buffer
     ...
     * @category Buffer
     */
    set stripIndexFormat(format) {
        if (validateStripIndex.includes(format)) {
            this.#stripIndexFormat = format;
            this.#update();
        }
        else
            consoleAndThrowError(`Invalid value for stripIndexFormat. Received ${format}. Expected one of: ${validateStripIndex.join(", ")}`);
    }
    /**
     * 프론트페이스(FrontFace) 반환
     * @category Buffer
    ...
    set frontFace(face) {
        if (validateFrontFaces.includes(face)) {
            ...
            consoleAndThrowError(`Invalid value for frontFace. Received ${face}. Expected one of: ${validateFrontFaces.join(", ")}`);
    }
    /**
     * 컬링 모드 반환
     * @category Buffer
    ...
    set cullMode(mode) {
        if (validateCullModes.includes(mode)) {
            ...
            consoleAndThrowError(`Invalid value for cullMode. Received ${mode}. Expected one of: ${validateCullModes.join(", ")}`);
    }
    /**
     * unclippedDepth 반환
     * @category Buffer
     ...
     * @category Buffer
     */
    set unclippedDepth(state) {
        if (typeof state === 'boolean') {
            this.#unclippedDepth = state;
            this.#update();
        }
        else
            consoleAndThrowError(`Invalid type for unclippedDepth. Received ${typeof state}. Expected type: boolean.`);
    }
    /**
