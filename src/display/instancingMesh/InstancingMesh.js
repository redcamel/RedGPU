...
class InstancingMesh extends Mesh {
     ...
     */
    constructor(redGPUContext, instanceCount, geometry, material) {
        super(redGPUContext, geometry, material);
        this.#redGPUContext = redGPUContext;
        this.gpuRenderInfo = new VertexGPURenderInfo(null, null, null, null, null, null, null, null);
    ...
    set instanceCount(count) {
        ...
        const newBuffer = new StorageBuffer(this.#redGPUContext, newData, `InstanceBuffer_${this.uuid}`);
        // 기존데이터
     ...
     */
    render(renderViewStateData, shadowRender = false) {
        ...
        if (this.geometry) {
            ...
            if (!this.gpuRenderInfo)
                this.#initGPURenderInfos(redGPUContext);
            const dirtyPipeline = this.dirtyPipeline || this.material.dirtyPipeline;
            ...
            const { vertexUniformBuffer, vertexUniformBindGroup, vertexUniformInfo, pipeline, shadowPipeline } = gpuRenderInfo;
            {
                //TODO 여기 개선
                if (vertexUniformInfo.members.displacementScale !== undefined) {
                    gpuDevice.queue.writeBuffer(vertexUniformBuffer.gpuBuffer, vertexUniformInfo.members.displacementScale.uniformOffset, new vertexUniformInfo.members.displacementScale.View([displacementScale]));
                }
                if (vertexUniformInfo.members.useDisplacementTexture !== undefined) {
                    gpuDevice.queue.writeBuffer(vertexUniformBuffer.gpuBuffer, vertexUniformInfo.members.useDisplacementTexture.uniformOffset, new vertexUniformInfo.members.useDisplacementTexture.View([displacementTexture ? 1 : 0]));
                }
            }
            if (this.dirtyTransform) {
                gpuDevice.queue.writeBuffer(vertexUniformBuffer.gpuBuffer, vertexUniformInfo.members.instanceGroupModelMatrix.uniformOffset, new vertexUniformInfo.members.instanceGroupModelMatrix.View(this.modelMatrix));
            }
     ...
     * @private
     */
    #initGPURenderInfos(redGPUContext) {
        this.dirtyPipeline = true;
        const { resourceManager } = this.#redGPUContext;
        const vertex_BindGroupLayout = resourceManager.getGPUBindGroupLayout(ResourceManager.PRESET_VERTEX_GPUBindGroupLayout_Instancing);
        ...
        const { material } = this;
        // BindGroup 을 생성합니다.
        const vertexBindGroupDescriptor = {
    ...
    #updatePipelines() {
        const { resourceManager, } = this.#redGPUContext;
        // 셰이더 모듈 설명자를 생성합니다.
        const vModuleDescriptor = { code: vertexModuleSource };
        const vertexShaderModule = resourceManager.createGPUShaderModule(VERTEX_SHADER_MODULE_NAME, vModuleDescriptor);
        const { vertexUniformBuffer } = this.gpuRenderInfo;
        const { material } = this;
        const { basicSampler, emptyBitmapTextureView, emptyCubeTextureView } = resourceManager;
        const { gpuSampler: basicGPUSampler } = basicSampler;
        const vertexBindGroupLayout = resourceManager.getGPUBindGroupLayout(ResourceManager.PRESET_VERTEX_GPUBindGroupLayout_Instancing);
        const vertexBindGroupDescriptor = {
