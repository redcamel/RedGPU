...
class Line3D extends Mesh {
     ...
     */
    constructor(redGPUContext, type = LINE_TYPE.LINEAR, baseColor = '#fff') {
        super(redGPUContext);
        this.primitiveState.topology = GPU_PRIMITIVE_TOPOLOGY.LINE_STRIP;
        this.baseColor = baseColor;
        this.#type = type;
        this._geometry = new Geometry(redGPUContext, new VertexBuffer(redGPUContext, this.#interleaveData, new VertexInterleavedStruct({
     ...
     */
    addPoint(x = 0, y = 0, z = 0, color = this.baseColor, colorAlpha = 1, inX = 0, inY = 0, inZ = 0, outX = 0, outY = 0, outZ = 0) {
        this.#originalPoints.push(new LinePointWithInOut(x, y, z, inX, inY, inZ, outX, outY, outZ, color, colorAlpha));
        this.#parsePointsByType();
    }
    /**
     ...
     * @param outZ out tangent Z (기본값: 0)
     */
    addPointAt(index, x = 0, y = 0, z = 0, color = this.baseColor, colorAlpha = 1, inX = 0, inY = 0, inZ = 0, outX = 0, outY = 0, outZ = 0) {
        if (this.#originalPoints.length < index)
            index = this.#originalPoints.length;
        if (index != undefined)
            this.#originalPoints.splice(index, 0, new LinePointWithInOut(x, y, z, inX, inY, inZ, outX, outY, outZ, color, colorAlpha));
        else
            this.#originalPoints.push(new LinePointWithInOut(x, y, z, inX, inY, inZ, outX, outY, outZ, color, colorAlpha));
        this.#parsePointsByType();
    }
    ...
    removePointAt(index) {
        ...
        if (this.#originalPoints[index])
            this.#originalPoints.splice(index, 1);
        else
            consoleAndThrowError('removeChildAt', 'index 해당인덱스에 위치한 포인트가 없음.', '입력값 : ' + index);
        this.#parsePointsByType();
    }
    /**
     * 모든 점을 삭제합니다.
        ...
        switch (this.#type) {
            ...
            case LINE_TYPE.BEZIER:
                if (originalPoints.length > 1) {
                    ...
                    this.#interleaveData.push(0, 0, 0, 1, 1, 1, 1);
                break;
            default:
                i = 0;
