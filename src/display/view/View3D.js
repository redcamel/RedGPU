...
class View3D extends AView {
     ...
     * @private
     */
    #systemUniform_Vertex_StructInfo = UNIFORM_STRUCT;
    /**
     ...
     */
    constructor(redGPUContext, scene, camera, name) {
     ...
     */
    get systemUniform_Vertex_StructInfo() {
     ...
     */
    get systemUniform_Vertex_UniformBindGroup() {
     ...
     */
    get systemUniform_Vertex_UniformBuffer() {
    ...
    update(view, shadowRender = false, calcPointLightCluster = false, renderPath1ResultTextureView) {
        ...
        if (index > -1) {
            ...
            if (prevInfo) {
                needResetBindGroup = (prevInfo.ibl !== ibl ||
     ...
     * @private
     */
    #createVertexUniformBindGroup(key, shadowDepthTextureView, ibl, renderPath1ResultTextureView) {
        ...
        const systemUniform_Vertex_BindGroupDescriptor = {
            ...
            entries: [
                ...
                {
                    binding: 10,
                    resource: resourceManager.getGPUResourceCubeTextureView(ibl_iblTexture, ibl_iblTexture?.viewDescriptor || CubeTexture.defaultViewDescriptor)
                },
                {
                    binding: 11,
     ...
     * @private
     */
    #updateIBLResourceStates(resourceManager, ibl_iblTexture, ibl_irradianceTexture) {
        // IBL 텍스처 쌍들을 배열로 정의
     ...
     * @private
     */
    #manageIBLResourceState(resourceManager, cacheKey, isAddingListener) {
     ...
     * @private
     */
    #updateClusters(calcClusterLight = false) {
        if (!calcClusterLight)
            return;
        ...
        if (scene) {
            ...
            if (pointLightNum) {
                ...
                while (i--) {
                    ...
                    if (tLight.enableDebugger) {
                        if (!tLight.drawDebugger)
                            tLight.drawDebugger = new DrawDebuggerPointLight(redGPUContext, tLight);
            ...
            if (spotLightNum) {
                ...
                while (i--) {
                    ...
                    if (tLight.enableDebugger) {
                        if (!tLight.drawDebugger)
                            tLight.drawDebugger = new DrawDebuggerSpotLight(redGPUContext, tLight);
                        tLight.drawDebugger.render(renderViewStateData);
                    }
                }
            }
            this.#clusterLightsBufferData.set([pointLightNum, spotLightNum, 0, 0], 0);
            this.redGPUContext.gpuDevice.queue.writeBuffer(this.#clusterLightsBuffer, 0, this.#clusterLightsBufferData);
