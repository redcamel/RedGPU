...
class SkyBox {
    ...
    set exposure(value) {
        ...
        if (this.#skyboxTexture instanceof HDRTexture)
            this.#skyboxTexture.exposure = value;
    }
    /**
     * 스카이박스의 불투명도를 반환합니다.
     ...
     * ```
     */
    transition(transitionTexture, duration = 300, transitionAlphaTexture) {
    ...
    #initGPURenderInfos(redGPUContext) {
        ...
        const vertex_BindGroupLayout = resourceManager.getGPUBindGroupLayout('SKYBOX_VERTEX_BIND_GROUP_LAYOUT') || resourceManager.createBindGroupLayout('SKYBOX_VERTEX_BIND_GROUP_LAYOUT', getVertexBindGroupLayoutDescriptorFromShaderInfo(SHADER_INFO, 1));
        // UniformBuffer
        const vertexUniformData = new ArrayBuffer(UNIFORM_STRUCT.arrayBufferByteLength);
        const vertexUniformBuffer = new UniformBuffer(redGPUContext, vertexUniformData, 'SKYBOX_VERTEX_UNIFORM_BUFFER', 'SKYBOX_VERTEX_UNIFORM_BUFFER');
        // modelMatrix
        ...
        this.gpuRenderInfo = new VertexGPURenderInfo(null, SHADER_INFO.shaderSourceVariant, SHADER_INFO.conditionalBlocks, UNIFORM_STRUCT, vertex_BindGroupLayout, vertexUniformBuffer, vertexUniformBindGroup, this.#updatePipeline());
    }
    /**
     * 렌더링 파이프라인을 업데이트합니다.
     *
    ...
    #updatePipeline() {
        ...
        const pipelineLayoutDescriptor = { bindGroupLayouts: bindGroupLayouts };
        const pipelineLayout = resourceManager.createGPUPipelineLayout('SKYBOX_PIPELINE_LAYOUT', pipelineLayoutDescriptor);
