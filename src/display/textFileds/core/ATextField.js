...
const isPixelNeeded = (key) => !['lineHeight', 'fontWeight'].includes(key);
/**
...
class ATextField extends Mesh {
    ...
    #needsUpdate = false; // 업데이트 플래그
    #renderWidth;
    #renderHeight;
    constructor(redGPUContext, imgOnload, mode3dYn = true) {
        super(redGPUContext);
    ...
    #createCanvasAndContext() {
        if (typeof OffscreenCanvas !== 'undefined')
            this.#textureCvs = new OffscreenCanvas(100, 100);
        else
            this.#textureCvs = document.createElement('canvas');
        this.#textureCtx = this.#textureCvs.getContext('2d');
    }
    #getRenderHtmlSize() {
        // 요소의 현재 크기 가져오기
    ...
    #setStyle = (key, baseValue) => {
        const svgDom = this.#svg.querySelector('foreignObject > div');
        const tStyle = svgDom.style;
        const tStyle2 = this.#htmlElement.style;
