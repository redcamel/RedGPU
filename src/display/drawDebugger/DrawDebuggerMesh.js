...
class DrawDebuggerMesh {
    ...
    #cachedOBB = null;
    #cachedAABB = null;
    constructor(redGPUContext, target) {
    ...
    #createWireframeBoxGeometry(redGPUContext) {
    ...
    #hasOBBChanged(currentOBB) {
        if (!this.#cachedOBB)
            return true;
        const cached = this.#cachedOBB;
        return (cached.center[0] !== currentOBB.center[0] ||
    ...
    #hasAABBChanged(currentAABB) {
        if (!this.#cachedAABB)
            return true;
        const cached = this.#cachedAABB;
        return (cached.minX !== currentAABB.minX ||
    ...
    #isMatrixEqual(a, b) {
        ...
        for (let i = 0; i < a.length; i++) {
            if (Math.abs(a[i] - b[i]) > 0.0001)
                return false; // 부동소수점 오차 허용
        }
        return true;
    }
    #cacheOBB(obb) {
        this.#cachedOBB = new OBB(obb.center, obb.halfExtents, obb.orientation);
    }
    #cacheAABB(aabb) {
        this.#cachedAABB = aabb.clone();
    }
    #updateVertexDataFromOBB(targetOBB, vertexBuffer) {
    ...
    #updateVertexDataFromAABB(targetAABB, vertexBuffer) {
    ...
    #updateVertexBuffer(transformedVertices, vertexBuffer) {
        const edges = [
            [0, 1], [1, 2], [2, 3], [3, 0], // 뒷면
            [4, 5], [5, 6], [6, 7], [7, 4], // 앞면
            [0, 4], [1, 5], [2, 6], [3, 7] // 연결선
