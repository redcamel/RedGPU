...
class DrawDebuggerAxis extends Mesh {
     ...
     * @private
     */
    #createAxis(geometry, color, scale, position) {
     ...
     * @private
     */
    #createConeLikeTip(redGPUContext, color, scale, // [radius, height]
    position, rotation) {
        const coneTip = new Mesh(redGPUContext, new Cylinder(redGPUContext, scale[0], 0.001, scale[1], 32, 1), // 윗면 반지름을 매우 작은 값으로 설정하여 원뿔 구현
