...
class DrawDebuggerDirectionalLight extends ADrawDebuggerLight {
    ...
    render(renderViewStateData) {
        if (!renderViewStateData.view.systemUniform_Vertex_UniformBindGroup)
            return;
        ...
        // 빛이 오는 방향 (화살표 반대편)에 레이블 배치
        this.#label.setPosition(visualPosition[0] - normalizedDir[0] * labelDistance, visualPosition[1] - normalizedDir[1] * labelDistance, visualPosition[2] - normalizedDir[2] * labelDistance);
    }
    #updateVertexDataFromDirectionalLight(light, vertexBuffer) {
        const visualPosition = this.#visualPosition;
