...
const createBasePipeline = (mesh, module, vertexBindGroupLayout, pipelineType) => {
    ...
    switch (pipelineType) {
        ...
        default:
            entryPoint = 'main';
            pipelineLabel = `${module.label}_pipeline`;
    }
    // 셰이더 모듈 설명자를 생성합니다.
    const vertexState = {
        module,
    ...
    if (pipelineType !== PIPELINE_TYPE.SHADOW)
        bindGroupLayouts.push(material_gpuRenderInfo.fragmentBindGroupLayout);
    const pipelineLayoutDescriptor = {
        bindGroupLayouts,
        label: `${pipelineLabel}_${material_gpuRenderInfo.fragmentBindGroupLayout.label}`
    };
    const pipelineLayout = resourceManager.createGPUPipelineLayout(`${pipelineLayoutDescriptor.label}`, pipelineLayoutDescriptor);
