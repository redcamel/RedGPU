...
const createMeshVertexShaderModule = (mesh) => {
    const { material } = mesh;
    let result;
    if (material instanceof PBRMaterial) {
        if (mesh.animationInfo.skinInfo) {
            ...
            result = mesh.createMeshVertexShaderModuleBASIC(VERTEX_SHADER_MODULE_NAME_PBR, SHADER_INFO_PBR, UNIFORM_STRUCT_PBR, vertexModuleSourcePbr);
    }
    else {
        if (mesh.createCustomMeshVertexShaderModule)
            result = mesh.createCustomMeshVertexShaderModule();
        else
            result = mesh.createMeshVertexShaderModuleBASIC(VERTEX_SHADER_MODULE_NAME_BASIC, SHADER_INFO_BASIC, UNIFORM_STRUCT_BASIC, vertexModuleSource);
    }
