...
const createMeshVertexShaderModulePBRSkin = (VERTEX_SHADER_MODULE_NAME_PBR_SKIN, mesh) => {
    ...
    const vModuleDescriptor = { code };
    console.log('createMeshVertexShaderModulePBRSkin', currentShaderModuleName);
    if (currentShaderModuleName === label)
        return resourceManager.getGPUShaderModule(label);
    else {
    ...
    const module = resourceManager.createGPUShaderModule(label, vModuleDescriptor);
