...
import Primitive, {IVolumeAABB} from "../../primitive/core/Primitive";
...
//
	...
	get volumeOBB(): IVolumeOBB {
		...
		const geometryVolume = this._geometry.volume;
		return this.#calculateOBB(geometryVolume);
	}
	get volumeAABB(): IVolumeAABB {
		...
		return this.#calculateAABBFromGeometry(geometryVolume);
	}
	/**
	 * geometryVolume을 기반으로 AABB 계산 (직접 변환)
	 */
	#calculateAABBFromGeometry(geometryVolume: IVolumeAABB): IVolumeAABB {
		...
		localVertices.forEach(vertex => {
			...
			vec3.transformMat4(worldVertex, localVertex, this.modelMatrix);
	 ...
	 */
	#calculateOBB(geometryVolume: IVolumeAABB):IVolumeOBB {
		// 원본 지오메트리의 중심점과 반크기
		...
		vec3.transformMat4(transformedCenter, originalCenter as vec3, this.modelMatrix);

		// 회전과 스케일 매트릭스 추출
		const rotationScaleMatrix = mat4.create();
		mat4.copy(rotationScaleMatrix, this.modelMatrix);
