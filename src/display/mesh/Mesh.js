...
const CPI = 3.141592653589793, CPI2 = 6.283185307179586, C225 = 0.225, C127 = 1.27323954, C045 = 0.405284735, C157 = 1.5707963267948966;
...
class Mesh extends MeshBase {
    ...
    #z = 0;
    /** 위치 배열 [x, y, z] */
    #positionArray = new Float32Array([0, 0, 0]);
    /** 피벗 X */
    #pivotX = 0;
    /** 피벗 Y */
    #pivotY = 0;
    ...
    #scaleZ = 1;
    /** 스케일 배열 [x, y, z] */
    #scaleArray = new Float32Array([1, 1, 1]);
    /** X축 회전 (deg) */
    #rotationX = 0;
    /** Y축 회전 (deg) */
    #rotationY = 0;
    ...
    #rotationZ = 0;
    /** 회전 배열 [x, y, z] (deg) */
    #rotationArray = new Float32Array([0, 0, 0]);
    /** 이벤트 핸들러 객체 */
    #events = {};
    /** 등록된 이벤트 개수 */
    #eventsNum = 0;
     ...
     * @param name 메시 이름(선택)
     */
    constructor(redGPUContext, geometry, material, name) {
        super(redGPUContext);
        if (name)
            this.name = name;
    ...
    set enableDebugger(value) {
        ...
        if (value && !this.#drawDebugger)
            this.#drawDebugger = new DrawDebuggerMesh(this.redGPUContext, this);
    }
    get drawDebugger() {
    ...
    set ignoreFrustumCulling(value) {
    ...
    setEnableDebuggerRecursively(enableDebugger = false) {
    ...
    setCastShadowRecursively(castShadow = false) {
    ...
    setReceiveShadowRecursively(receiveShadow = false) {
            ...
            this.#needUpdateNormal = true;
            {
                ...
                tLocalMatrix[11] = tLocalMatrix[11] * aZ;
                {
                    ...
                    a00 = tLocalMatrix[0], a01 = tLocalMatrix[1], a02 = tLocalMatrix[2], a03 = tLocalMatrix[3],
                        ...
                        tLocalMatrix[15] = b0 * a03 + b1 * a13 + b2 * a23 + b3 * a33) : 0;
                }
                {
    ...
    createMeshVertexShaderModuleBASIC = (VERTEX_SHADER_MODULE_NAME, SHADER_INFO, UNIFORM_STRUCT_BASIC, vertexModuleSource) => {
        const { redGPUContext } = this;
        const { gpuRenderInfo } = this;
        ...
        if (!targetShaderModule) {
            ...
            if (variantSource) {
                ...
                if (this.animationInfo?.skinInfo) {
                    ...
                    targetShaderModule = resourceManager.createGPUShaderModule(`${variantShaderModuleName}_${jointNum}`, { code: variantSource });
                }
                else {
