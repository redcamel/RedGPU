...
class LandscapeWeightMapCache {
            ...
            const tryLoad = (useCors: boolean) => {
                ...
                img.onload = () => {
                    try {
                        ...
                        if (useCors) {
                            // CORS / Tainted canvas 에러 시 CORS 없이 재시도
                            tryLoad(false);
                        } else {
    ...
    /**
     * [KO] UV 좌표 (0.0 ~ 1.0)와 채널 인덱스(0:R, 1:G, 2:B, 3:A)로부터 4-Tap Bilinear(선형 보간) 가중치(0.0 ~ 1.0)를 O(1) Zero-GC로 조회합니다.
     * [EN] Queries 4-Tap Bilinear interpolated weight (0.0 ~ 1.0) from UV coordinates and channel index in O(1) Zero-GC.
     */
    static getWeight(src: string, u: number, v: number, channelIndex: number = 0): number {
        ...
        if (!entry) return 0.0; // 캐시 로드 전에는 안전하게 0.0 (스폰 보류 - 로드 완료 후 자동 재배치)
    ...
    /**
     * [KO] 단일 스플랫맵 텍스처의 4개 채널(RGBA) 가중치를 Bilinear 보간하여 전달받은 버퍼(outWeights)에 Zero-GC로 저장합니다.
     * [EN] Bilinear samples 4 channels (RGBA) of single weightmap texture into provided buffer (outWeights) with Zero-GC.
     */
