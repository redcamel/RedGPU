 ...
 *
 */
const initOffscreen = async (canvas, onWebGPUInitialized, onFailInitialized, onDestroy, alphaMode = 'premultiplied', requestAdapterOptions = {
    ...
    const errorHandler = (e, defaultMsg) => {
        const msg = generateErrorMessage(e, defaultMsg);
        console.error('\n============\n', msg, '\n============\n');
        onFailInitialized?.(msg);
    };
    const validateAndRequestAdapter = async (targetGPU) => {
        if (!targetGPU)
            errorHandler(null, `Cannot find navigator.gpu`);
        try {
            const adapter = await targetGPU.requestAdapter(requestAdapterOptions);
    ...
    const validateAndRequestDevice = async (adapter) => {
    ...
    const validateAndInitializeContext = (canvas, adapter, device) => {
        ...
        try {
            const redGPUContext = new RedGPUContext(canvas, offscreenCanvas, adapter, device, context, alphaMode);
...
const generateErrorMessage = (e, defaultMsg) => {
    let msg = defaultMsg;
