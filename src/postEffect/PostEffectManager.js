...
class PostEffectManager {
    ...
    /** Compute 셰이더 모듈 */
    #textureComputeShaderModule;
    /** Compute 바인드 그룹 */
    #textureComputeBindGroup;
    /** Compute 바인드 그룹 레이아웃 */
    #textureComputeBindGroupLayout;
    /** Compute 파이프라인 */
    #textureComputePipeline;
    /** 이전 프레임 텍스처 크기 */
    #previousDimensions;
    /** 시스템 유니폼 버퍼 */
    ...
    get postEffectSystemUniformBuffer() {
    ...
    render() {
        ...
        this.#postEffects.forEach(effect => {
            currentTextureView = effect.render(this.#view, width, height, currentTextureView);
        });
        if (useFXAA) {
            currentTextureView = fxaa.render(this.#view, width, height, currentTextureView);
        }
        if (useTAA) {
            currentTextureView = taa.render(this.#view, width, height, currentTextureView);
        }
    ...
    #renderToStorageTexture(view, sourceTextureView) {
        ...
        this.#executeComputePass(gpuDevice, this.#textureComputePipeline, this.#textureComputeBindGroup, width, height);
    ...
    #createTextureBindGroupLayout(redGPUContext) {
    ...
    #createStorageTexture(gpuDevice, width, height) {
    ...
    #createTextureBindGroup(redGPUContext, bindGroupLayout, sourceTextureView, storageTextureView) {
    ...
    #createTextureComputePipeline(gpuDevice, shaderModule, bindGroupLayout) {
    ...
    #executeComputePass(gpuDevice, pipeline, bindGroup, width, height) {
