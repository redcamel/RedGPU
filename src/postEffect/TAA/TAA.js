...
class TAA {
    ...
    #prevInfo;
    #cachedBindGroupLayouts = new Map();
    #cachedPipelineLayouts = new Map();
    #cachedComputePipelines = new Map();
    #currentMSAAState = null;
    // 스왑 버퍼 구조로 변경
    #currentFrameTexture;
    #currentFrameTextureView;
    #previousFrameTexture;
    #previousFrameTextureView;
    #frameBufferBindGroup0;
    #frameBufferBindGroup1;
    #WORK_SIZE_X = 8;
    #WORK_SIZE_Y = 8;
    #WORK_SIZE_Z = 1;
    ...
    constructor(redGPUContext) {
        ...
        this.#init(redGPUContext, 'POST_EFFECT_TAA', {
    ...
    get currentFrameTextureView() {
        return this.#currentFrameTextureView;
    }
    get temporalBlendFactor() {
        return this.#temporalBlendFactor;
    }
    set temporalBlendFactor(value) {
    ...
    render(view, width, height, sourceTextureInfo) {
        ...
        this.#execute(gpuDevice, width, height);
        {
            ...
            commentEncode_compute.copyTextureToTexture({ texture: this.#currentFrameTexture }, { texture: this.#previousFrameTexture }, [width, height, 1]);
    ...
    #init(redGPUContext, name, computeCodes) {
        this.#name = name;
        ...
        if (this.#uniformsInfo) {
            ...
            this.#uniformBuffer = new UniformBuffer(redGPUContext, uniformData, `TAA_UniformBuffer`);
        }
    }
    ...
    #createFrameBufferBindGroups(view, sourceTextureView, useMSAA, redGPUContext, gpuDevice) {
        const computeBindGroupEntries0 = [];
        const computeBindGroupEntries1 = [];
    ...
    #createBindGroups(entries0, entries1, useMSAA, redGPUContext, gpuDevice) {
        ...
        if (!this.#cachedBindGroupLayouts.has(layoutKey0)) {
            const layout0 = redGPUContext.resourceManager.getGPUBindGroupLayout(layoutKey0) ||
                redGPUContext.resourceManager.createBindGroupLayout(layoutKey0, getComputeBindGroupLayoutDescriptorFromShaderInfo(currentShaderInfo, 0, useMSAA));
        ...
        if (!this.#cachedBindGroupLayouts.has(layoutKey1)) {
            const layout1 = redGPUContext.resourceManager.getGPUBindGroupLayout(layoutKey1) ||
                redGPUContext.resourceManager.createBindGroupLayout(layoutKey1, getComputeBindGroupLayoutDescriptorFromShaderInfo(currentShaderInfo, 1, useMSAA));
    ...
    #createComputePipeline(useMSAA, redGPUContext, gpuDevice) {
    ...
    #detectSourceTextureChange(sourceTextureView) {
