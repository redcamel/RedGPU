...
// [KO] 2. 업계 표준 1D 분리형 가우시안 블러 수행 (X축)
// [EN] 2. Industry standard 1D Separable Gaussian Blur execution (X-axis)
...
for (var i = 1.0; i <= steps; i += 1.0) {
    ...
    let w1 = exp(-(x1 * x1) / twoSigmaSq);
    let w2 = exp(-(x2 * x2) / twoSigmaSq);
    let w = w1 + w2;

    if (w < 0.0001) { break; }

    let offsetPixels = (x1 * w1 + x2 * w2) / w;
    let offsetUV = vec2<f32>(offsetPixels * invSize.x, 0.0);
