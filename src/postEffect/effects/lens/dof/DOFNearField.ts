			...
			fn main (
			  @builtin(global_invocation_id) global_id : vec3<u32>,
				...
				if (cocValue >= 0.0) {
					...
					textureStore(outputTexture, index, vec4<f32>(originalColor.rgb, 1.0));
					return;
				}
				
				let blurRadius = abs(cocValue) * uniforms.blurSize;
				...
				let maxRadius = min(blurRadius, 16.0);
				let samples = 16;
				...
				for (var i = 0; i < samples; i = i + 1) {
					for (var r = 1.0; r <= maxRadius; r = r + 1.0) {
						...
						if (sampleCoC < 0.0 && abs(sampleCoC) >= abs(cocValue) * 0.8) {
							weight *= 1.5;
						}
				...
				if (totalWeight > 0.0) {
					sum /= totalWeight;
					textureStore(outputTexture, index, vec4<f32>(sum, 1.0));
				} else {
					let originalColor = textureLoad(sourceTexture0, index);
					textureStore(outputTexture, index, vec4<f32>(originalColor.rgb, 1.0));
				}
...
Object.freeze(DOFNearField)
export default DOFNearField
