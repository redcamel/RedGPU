...
class FilmGrain extends ASinglePassPostEffect {
    ...
    /** 그레인 강도. 0~1, 기본값 0.12 */
    #filmGrainIntensity = HEAVY.filmGrainIntensity;
    /** 밝기 반응도. 0~2, 기본값 0.6 */
    #filmGrainResponse = HEAVY.filmGrainResponse;
    /** 그레인 스케일. 0.1~20, 기본값 4.0 */
    #filmGrainScale = HEAVY.filmGrainScale;
    /** 컬러 그레인 비율. 0~1, 기본값 0.7 */
    #coloredGrain = HEAVY.coloredGrain;
    /** 그레인 채도. 0~2, 기본값 0.8 */
    #grainSaturation = HEAVY.grainSaturation;
    #time = 0.0;
    #devicePixelRatio = 1.0;
    constructor(redGPUContext) {
        super(redGPUContext);
        this.#devicePixelRatio = window?.devicePixelRatio || 1.0;
        this.init(redGPUContext, 'POST_EFFECT_FILM_GRAIN', createBasicPostEffectCode(this, computeCode, uniformStructCode));
        this.#updateUniforms();
    }
    /** 그레인 강도 반환 */
    get filmGrainIntensity() {
        return this.#filmGrainIntensity;
    }
    /** 그레인 강도 설정. 0~1 */
    set filmGrainIntensity(value) {
        this.#filmGrainIntensity = Math.max(0.0, Math.min(1.0, value));
        this.updateUniform('filmGrainIntensity', this.#filmGrainIntensity);
    }
    /** 밝기 반응도 반환 */
    get filmGrainResponse() {
        return this.#filmGrainResponse;
    }
    /** 밝기 반응도 설정. 0~2 */
    set filmGrainResponse(value) {
