...
const BufferDetailModal = ({item, type, onClose}: { item: any, type: string, onClose: () => void }) => {
    ...
    const label = isRaw ? item.label : (item.label || buf?.label || buf?.name || 'Unnamed Buffer');
        ...
        if (type === 'vertexBuffer' && stride) {
            ...
            return (
                <div style={{display: 'flex', flexDirection: 'column', gap: '6px', width: '100%', overflowX: 'hidden'}}>
                    ...
                    {f32.length > limit &&
                        <div style={moreStyle}>... and {formatNumber(f32.length - limit, 0)} more</div>}
                </div>
            );
        }
        ...
        if (type === 'indexBuffer') {
            ...
            return (
                <div
                    ...
                    {u32.length > limit &&
                        <div style={moreStyle}>... and {formatNumber(u32.length - limit, 0)} more</div>}
                </div>
            );
        }
